      SUBROUTINE SMATRIX_GDX_NINJDX(P1,ANS)
C  
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF(GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATRIX_GDX_NINJDX(P,NHEL(1,IHEL),JC(1))            
      ...
      REAL*8 FUNCTION MATRIX_GDX_NINJDX(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      ! diagram 11 <- resonant
      CALL HVSXXX(W(1,1),W(1,17),GC,MDL,WDLR,W(1,21))     
      CALL IOSXXX(W(1,5),W(1,16),W(1,21),GDLNJM,AMP(11))         
      ! diagram 12 <- resonant
      CALL HVSXXX(W(1,1),W(1,19),GC,MDR,WDRR,W(1,22))     
...
c delete resonant diagrams (diagram removal type I)
     ...
     &             +AMP(  11)+AMP(  12)+AMP(  13)+AMP(  14)
      MATRIX_GDX_NINJDX = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP =(0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATRIX_GDX_NINJDX =MATRIX_GDX_NINJDX
...
c (diagram removal type II)
     ...
     &                      +ZTEMP*DCONJG(JAMPR(I))/DENOM(I)   
      ...
      MATRIX_GDX_NINJDX = MATRIX_GDX_NINJDX - MATRIX_RESONANT
#endif
...
c back in subroutine setosresreal
      ...
      MATRIX_GDX_NINJDX = MATRIX_GDX_NINJDX
     &                - MATRIX_GDX_NINJDX_RES(p,NHEL,IC,"dl35")
     &                - MATRIX_GDX_NINJDX_RES(p,NHEL,IC,"dr35")
     &                - MATRIX_GDX_NINJDX_RES(p,NHEL,IC,"dl45")
     &                - MATRIX_GDX_NINJDX_RES(p,NHEL,IC,"dr45")
#endif
...
c the interference term and the off-shell contributions
      ...
      if( (S.ge.(MDL+dabs(MNJ))**2) .and. (MDL.ge.dabs(MNI))) then
        ...
        COUNTER35L = RATIO35L*MATRIX_GDX_NINJDX_RES(p_OS,NHEL,IC,"dl35") ! generate the counter term
      endif
      ...
      if( (S.ge.(MDR+dabs(MNJ))**2) .and. (MDR.ge.dabs(MNI))) then
        ...
        COUNTER35R = RATIO35R*MATRIX_GDX_NINJDX_RES(p_OS,NHEL,IC,"dr35")
      endif
      ...
      if( (S.ge.(MDL+dabs(MNI))**2) .and. (MDL.ge.dabs(MNJ))) then
        ...
        COUNTER45L = RATIO45L*MATRIX_GDX_NINJDX_RES(p_OS,NHEL,IC,"dl45")
      endif
      ...
      if( (S.ge.(MDR+dabs(MNI))**2) .and. (MDR.ge.dabs(MNJ))) then
        ...
        COUNTER45R = RATIO45R*MATRIX_GDX_NINJDX_RES(p_OS,NHEL,IC,"dr45")
      endif

      MATRIX_GDX_NINJDX = MATRIX_GDX_NINJDX - COUNTER35L - COUNTER35R
