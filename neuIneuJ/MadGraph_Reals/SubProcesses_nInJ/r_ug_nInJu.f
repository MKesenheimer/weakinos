      SUBROUTINE SMATRIX_UG_NINJU(P1,ANS)
C  
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF(GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATRIX_UG_NINJU(P,NHEL(1,IHEL),JC(1))            
      ...
      REAL*8 FUNCTION MATRIX_UG_NINJU(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
c the interference term and the off-shell contributions
      ...
      if( (S.ge.(MUL+dabs(MNJ))**2) .and. (MUL.ge.dabs(MNI))) then
        ...
        COUNTER35L = RATIO35L*MATRIX_UG_NINJU_RES(p_OS,NHEL,IC,"ul35") ! generate the counter term
      endif
      ...
      if( (S.ge.(MUR+dabs(MNJ))**2) .and. (MUR.ge.dabs(MNI))) then
        ...
        COUNTER35R = RATIO35R*MATRIX_UG_NINJU_RES(p_OS,NHEL,IC,"ur35")
      endif
      ...
      if( (S.ge.(MUL+dabs(MNI))**2) .and. (MUL.ge.dabs(MNJ))) then
        ...
        COUNTER45L = RATIO45L*MATRIX_UG_NINJU_RES(p_OS,NHEL,IC,"ul45")
      endif
      ...
      if( (S.ge.(MUR+dabs(MNI))**2) .and. (MUR.ge.dabs(MNJ))) then
        ...
        COUNTER45R = RATIO45R*MATRIX_UG_NINJU_RES(p_OS,NHEL,IC,"ur45")
      endif
      
      MATRIX_UG_NINJU = MATRIX_UG_NINJU - COUNTER35L - COUNTER35R
