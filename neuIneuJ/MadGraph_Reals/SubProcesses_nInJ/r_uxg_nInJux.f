      SUBROUTINE SMATRIX_UXG_NINJUX(P1,ANS)
C  
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF(GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATRIX_UXG_NINJUX(P,NHEL(1,IHEL),JC(1))            
      ...
      REAL*8 FUNCTION MATRIX_UXG_NINJUX(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
c delete resonant diagrams (diagram removal type I)
     ...
     &             +AMP(  11)+AMP(  12)+AMP(  13)+AMP(  14)
      MATRIX_UXG_NINJUX = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP =(0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATRIX_UXG_NINJUX =MATRIX_UXG_NINJUX
...
c (diagram removal type II)
     ...
     &                      +ZTEMP*DCONJG(JAMPR(I))/DENOM(I)   
      ...
      MATRIX_UXG_NINJUX = MATRIX_UXG_NINJUX - MATRIX_RESONANT
#endif
...
c back in subroutine setosresreal
      ...
      MATRIX_UXG_NINJUX = MATRIX_UXG_NINJUX
     &                - MATRIX_UXG_NINJUX_RES(p,NHEL,IC,"ul35")
     &                - MATRIX_UXG_NINJUX_RES(p,NHEL,IC,"ur35")
     &                - MATRIX_UXG_NINJUX_RES(p,NHEL,IC,"ul45")
     &                - MATRIX_UXG_NINJUX_RES(p,NHEL,IC,"ur45")
#endif
...
c the interference term and the off-shell contributions
      ...
      if( (S.ge.(MUL+dabs(MNJ))**2) .and. (MUL.ge.dabs(MNI))) then
        ...
        COUNTER35L = RATIO35L*MATRIX_UXG_NINJUX_RES(p_OS,NHEL,IC,"ul35") ! generate the counter term
      endif
      ...
      if( (S.ge.(MUR+dabs(MNJ))**2) .and. (MUR.ge.dabs(MNI))) then
        ...
        COUNTER35R = RATIO35R*MATRIX_UXG_NINJUX_RES(p_OS,NHEL,IC,"ur35")
      endif
      ...
      if( (S.ge.(MUL+dabs(MNI))**2) .and. (MUL.ge.dabs(MNJ))) then
        ...
        COUNTER45L = RATIO45L*MATRIX_UXG_NINJUX_RES(p_OS,NHEL,IC,"ul45")
      endif
      ...
      if( (S.ge.(MUR+dabs(MNI))**2) .and. (MUR.ge.dabs(MNJ))) then
        ...
        COUNTER45R = RATIO45R*MATRIX_UXG_NINJUX_RES(p_OS,NHEL,IC,"ur45")
      endif
      
      MATRIX_UXG_NINJUX = MATRIX_UXG_NINJUX - COUNTER35L - COUNTER35R
