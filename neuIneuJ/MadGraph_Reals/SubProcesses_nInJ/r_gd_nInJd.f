      SUBROUTINE SMATRIX_GD_NINJD(P1,ANS)
C  
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF(GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATRIX_GD_NINJD(P,NHEL(1,IHEL),JC(1))            
      ...
      REAL*8 FUNCTION MATRIX_GD_NINJD(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
c the interference term and the off-shell contributions
      ...
      if( (S.ge.(MDL+dabs(MNJ))**2) .and. (MDL.ge.dabs(MNI))) then
        ...
        COUNTER35L = RATIO35L*MATRIX_GD_NINJD_RES(p_OS,NHEL,IC,0) ! generate the counter term
      endif
      ! S35 = MDR^2, m3 = MNI, m5 = 0D0
      if( (S.ge.(MDR+dabs(MNJ))**2) .and. (MDR.ge.dabs(MNI))) then
        call off_to_on(p,3,5,4,MDR,p_OS)
        RATIO35R = (MDR*WREG)**2/((S35-MDR**2)**2+(MDR*WREG)**2)
        COUNTER35R = RATIO35R*MATRIX_GD_NINJD_RES(p_OS,NHEL,IC,1)
      endif
      ...
      if( (S.ge.(MDL+dabs(MNI))**2) .and. (MDL.ge.dabs(MNJ))) then
        ...
        COUNTER45L = RATIO45L*MATRIX_GD_NINJD_RES(p_OS,NHEL,IC,2)
      endif
      ! S45 = MDR^2, m4 = MNJ, m5 = 0D0
      if( (S.ge.(MDR+dabs(MNI))**2) .and. (MDR.ge.dabs(MNJ))) then
        call off_to_on(p,4,5,3,MDR,p_OS)
        RATIO45R = (MDR*WREG)**2/((S45-MDR**2)**2+(MDR*WREG)**2)
        COUNTER45R = RATIO45R*MATRIX_GD_NINJD_RES(p_OS,NHEL,IC,3)
      endif

      MATRIX_GD_NINJD = MATRIX_GD_NINJD - COUNTER35L - COUNTER35R
