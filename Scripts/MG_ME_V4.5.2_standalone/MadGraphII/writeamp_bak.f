!WTARO!: to add helicities of a spin-3/2 particle.
...
c      if(name(1:5).ne.'DIPOL') DipolSub=.false.
      jchar = jchar+ishift
      if((Dipolsub.eqv..false.).and.(DipolsubQED.eqv..false.)) then
       ishift= 10
       buff(jchar:jchar+ishift) = '(P,NHEL,IC)'
      endif
      if(Dipolsub.or.DipolSubQED) then
       if(offdiag) then
        ishift=28
...
c      write(buff(46:46),'(i1)') nexternal
      ...
      if(.not.standalone.and..not.DipolSub.and..not.DipolSubQED)then
         write(ln,10) 'include "genps.inc"'
         write(ln,10) 'include "nexternal.inc"'
         write(ln,10) 'include "maxamps.inc"'
      endif
      if(standalone.or.DipolSub.or.DipolSubQED)
     .            write(ln,10) 'include "nexternal.inc"'
      if(DipolSub.or.DipolSubQED)write(ln,10) 'include "dipole.inc"'
...
! write argument declarations
      ...
      if((Dipolsub.and.(.not.offdiag)).or.
     &    (DipolsubQED.and.(.not.offdiag))) then
         write(ln,10) 'INTEGER NHEL(NEXTERNAL), IC(NEXTERNAL)'
      elseif((Dipolsub.and.offdiag).or.(DipolsubQED.and.offdiag)) then
...
! write global declarations
      ...
      write(ln,5) ' '
      if(.not.standalone.and..not.DipolSub.and..not.DipolSubQED) then
      write(92,10) 'Double Precision amp2(maxamps), jamp2(0:maxflow)'
...
c     $                 ,den(j)/den(i),'*AMP(',j,')'
     ...
     &        ,',W1,W(1,',info_ijk(dipol,1,4),'),SUB,SUBV)'
      ...
      if(DipolSubQED) then
        if(chargeinfo(info_ijk(dipol,1,1)).eq.2d0/3d0) then
            chargint(dipol,1)=23
        elseif(chargeinfo(info_ijk(dipol,1,1)).eq.-2d0/3d0) then
            chargint(dipol,1)=-23
        elseif(chargeinfo(info_ijk(dipol,1,1)).eq.1d0/3d0) then
            chargint(dipol,1)=13
        elseif(chargeinfo(info_ijk(dipol,1,1)).eq.-1d0/3d0) then
            chargint(dipol,1)=-13
        elseif(chargeinfo(info_ijk(dipol,1,1)).eq.1d0) then
            chargint(dipol,1)=1
        elseif(chargeinfo(info_ijk(dipol,1,1)).eq.-1d0) then
            chargint(dipol,1)=-1
        endif
        if(chargeinfo(info_ijk(dipol,1,3)).eq.2d0/3d0) then
            chargint(dipol,2)=23
        elseif(chargeinfo(info_ijk(dipol,1,3)).eq.-2d0/3d0) then
            chargint(dipol,2)=-23
        elseif(chargeinfo(info_ijk(dipol,1,3)).eq.1d0/3d0) then
            chargint(dipol,2)=13
        elseif(chargeinfo(info_ijk(dipol,1,3)).eq.-1d0/3d0) then
            chargint(dipol,2)=-13
        elseif(chargeinfo(info_ijk(dipol,1,3)).eq.1d0) then
            chargint(dipol,2)=1
        elseif(chargeinfo(info_ijk(dipol,1,3)).eq.-1d0) then
            chargint(dipol,2)=-1
        endif

c this is different in the case of a->f fbar splitting
        if(info_ijk(dipol,1,1).gt.nincoming.and.(info_ijk(dipol,2,1)
     &     .eq.info_ijk(dipol,2,2)) ) then
          chargint(dipol,2)=-chargint(dipol,1)
        elseif(info_ijk(dipol,1,1).le.nincoming.and.(info_ijk(dipol,2,1)
     &     .eq.info_ijk(dipol,2,2)) ) then
          if(chargeinfo(info_ijk(dipol,1,2)).eq.2d0/3d0) then
              chargint(dipol,2)=23
          elseif(chargeinfo(info_ijk(dipol,1,2)).eq.-2d0/3d0) then
              chargint(dipol,2)=-23
          elseif(chargeinfo(info_ijk(dipol,1,2)).eq.1d0/3d0) then
              chargint(dipol,2)=13
          elseif(chargeinfo(info_ijk(dipol,1,2)).eq.-1d0/3d0) then
              chargint(dipol,2)=-13
          elseif(chargeinfo(info_ijk(dipol,1,2)).eq.1d0) then
              chargint(dipol,2)=1
          elseif(chargeinfo(info_ijk(dipol,1,2)).eq.-1d0) then
              chargint(dipol,2)=-1
          endif
          chargint(dipol,1)=-chargint(dipol,2)
        elseif(info_ijk(dipol,1,1).le.nincoming.and.(info_ijk(dipol,2,1)
     &     .eq.0.and.info_ijk(dipol,2,2).eq.1) ) then
          if(chargeinfo(info_ijk(dipol,1,2)).eq.2d0/3d0) then
              chargint(dipol,2)=23
          elseif(chargeinfo(info_ijk(dipol,1,2)).eq.-2d0/3d0) then
              chargint(dipol,2)=-23
...
c         write(92,44) 'CALL DIPOLESUBQED(P1,',info_ijk(dipol,1,1),','
c     &        ,info_ijk(dipol,1,2),',',info_ijk(dipol,1,3),','
c     &        ,info_ijk(dipol,1,4),',',info_ijk(dipol,1,5),','
c     &        ,info_ijk(dipol,2,1),',',info_ijk(dipol,2,2),','
c     &        ,info_ijk(dipol,2,3),',',info_ijk(dipol,2,4),','
c     &        ,info_ijk(dipol,2,5),',',chargint(1),',',chargint(2),','
c         write(92,41) mass(info_ijk(dipol,1,1)),','
c     &        ,mass(info_ijk(dipol,1,2)),',',mass(info_ijk(dipol,1,3))
c     &        ,',w(1,1),w(1,1),SUB,SUBV)'
      endif

      if (DipolSubQED.and..not.offdiag) then
         write(92,44) 'CALL DIPOLESUBQED(P1,',info_ijk(dipol,1,1),','
     &        ,info_ijk(dipol,1,2),',',info_ijk(dipol,1,3),','
     &        ,info_ijk(dipol,1,4),',',info_ijk(dipol,1,5),','
     &        ,info_ijk(dipol,2,1),',',info_ijk(dipol,2,2),','
     &        ,info_ijk(dipol,2,3),',',info_ijk(dipol,2,4),','
     &        ,info_ijk(dipol,2,5),',',chargint(dipol,1),',',chargint(dipol,2),','
         write(92,41) mass(info_ijk(dipol,1,1)),','
     &        ,mass(info_ijk(dipol,1,2)),',',mass(info_ijk(dipol,1,3))
     &        ,',w(1,1),w(1,1),SUB,SUBV)'
      elseif(DipolSubQED.and.offdiag) then
         buff='CALL VXXXXX(P(0,??),ZERO,-NHEL(??),??*IC(??),W1)'
         write(buff(17:18),'(i2)') info_ijk(dipol,1,4)
         write(buff(32:33),'(i2)') info_ijk(dipol,1,4)
         if(info_ijk(dipol,1,4).le.nincoming) then
     ...
     &        ,',W1,W(1,',info_ijk(dipol,1,4),'),SUB,SUBV)'
      ...
      if(DipolSub.or.DipolSubQED) then
       write(ln,10) 'IF (INTDIPOLE .EQV. .TRUE.) THEN'
       write(ln,10) ' SUB=1d0'
       write(ln,10) ' SUBV=(0d0,0d0)'
       write(ln,10) 'ENDIF'
      endif
...
c      write(ln,10) '        IMAX  = MAX(IMAX,ABS(DIMAG(ZTEMP)))'
        write(ln,10) '    ENDDO'
      if((Dipolsub.or.DipolsubQED).and.offdiag) then
     ...
     .        '+ZTEMP*DCONJG(JAMP(I))/DENOM(I)'
      ...
      if((Dipolsub.and.offdiag).or.(DipolsubQED.and.offdiag)) then
           write(ln,10)
...
!>>>>>>> 1.14.2.5
 ...
 44   format(6X,A,10(I2,A),2(I3,A),2(I2,A))
      end
...
c
      ...
      do i=1,ncross
         if (imatch(i) .gt. icross) then
            ...
            if(DipolSub.or.DipolSubQED) iden(imatch(i)) = isymn1*initc*jhel
      ...
      do i=1,ncross
         ...
         do j=1,2
            ...
            if(DipolSub.or.DipolSubQED) then
               ...
               if(str1(1:1) .ne. 'u' .and.str1(1:1) .ne. 'd'
     &              .and.str1(1:1) .ne. 's' .and. str1(1:1) .ne. 'c'
     &              .and.str1(1:1) .ne. 't' .and. str1(1:1) .ne. 'b'
     &              .and.str1(1:1) .ne. 'g') then
                  part_type(dipol,j)(1:4)='nqcd'
               endif
            endif
...
! write parameters
      ...
      if(DipolSub) then
       if(info_ijk(dipol,2,1).eq.info_ijk(dipol,2,2)) then
        offdiag=.true.
       else
        offdiag=.false.
       endif
      ...
      if(.not.standalone.and..not.DipolSub.and..not.DipolSubQED)then
         ...
         write(92,10) 'Include "maxamps.inc"'
      endif
      if(standalone.or.DipolSub.or.DipolSubQED)
     .        write(92,10) 'include "nexternal.inc"'
      if(DipolSub.or.DipolSubQED) write(92,10) 'include "dipole.inc"'
      if(DipolSub.or.DipolSubQED) write(92,10) 'include "coupl.inc"'
...
c      write(buff(43:45),'(i3)') ncross
      ...
      if(onium) then
        ...
        write(92,10) 'INTEGER NHEL(NEXTERNAL,NCOMB),NTRY'
        write(92,10) 'REAL*8 T, P(0:3,NEXTERNAL)'
      if ((DipolSub .or. DipolSubQED) .and. .not.offdiag) 
     &       write (92,10) 'REAL*8 P2(0:3,NEXTERNAL)'
        buff = 'REAL*8 '
      ...
      if(standalone.or.DipolSub.or.DipolSubQED)  write(92,10) 'DATA NTRY/0/'
...
c      write(92,10) buff(1:iname+7)
     ...
     &    ',',mass(info_ijk(dipol,1,3)),')'
         write(92,10) 'ELSE'
         write(92,40) ' CALL TRANSFORMINT(P1,P,',info_ijk(dipol,1,1),','
     &   ,info_ijk(dipol,1,2),',',info_ijk(dipol,1,3),','
     &   ,info_ijk(dipol,1,4),',',info_ijk(dipol,1,5),
     &    ',',mass(info_ijk(dipol,1,1)),',',mass(info_ijk(dipol,1,2)),
     &    ',',mass(info_ijk(dipol,1,3)),')'
      ...
      if(DipolSubQED) then
         write(92,10) 'IF(INTDIPOLE .EQV. .FALSE.) THEN'
         write(92,40) ' CALL TRANSFORMQED(P1,P,',info_ijk(dipol,1,1),','
     &   ,info_ijk(dipol,1,2),',',info_ijk(dipol,1,3),','
     &   ,info_ijk(dipol,1,4),',',info_ijk(dipol,1,5),
     &    ',',mass(info_ijk(dipol,1,1)),',',mass(info_ijk(dipol,1,2)),
     &    ',',mass(info_ijk(dipol,1,3)),')'
...
c
      ...
      write(92,11)  '         GOODHEL(IHEL,IPROC)=.TRUE.'
       if(.not.standalone.and..not.DipolSub.and..not.DipolSubQED) 
...
c*****************KEK1**************************
         ...
         ql=0
c****************WTARO**************************
         elseif(step(ipart).eq.-1) then 
         ql=-1
         elseif(step(ipart).le.-2) then
         ql=-2
c****************WTARO**************************
         endif 
c****************KEK1***************************
...
c*****************KEK2**************************
         ...
         ql2=1
c****************WTARO**************************
         elseif(step(iflip).eq.-1) then
         ql=-1  
         ql2=-1
         elseif(step(iflip).le.-2) then
         ql=-2
         ql2=-1
c****************WTARO**************************
         endif 
