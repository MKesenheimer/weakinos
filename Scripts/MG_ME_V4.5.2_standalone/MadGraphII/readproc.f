...
c      possible = .true.
      ...
      Subroutine get_photonpdf
c**********************************************************************
c     Determines whether a pdf should be used for electroweak dipoles
c     if there are photons in the initial state.
c**********************************************************************
      implicit none
      include 'params.inc'
      ...
      read(*,'(a)',end=10) ,buff
       ...
       do i=1,50
         if(index(buff(i:i+4),"TRUE").gt.0) then
           photonpdf=.true.
           found=.true.
          elseif(index(buff(i:i+5),"FALSE").gt.0) then
           photonpdf=.false.
           found=.true.
          endif
        enddo

        if (index(buff,"END_DIPOLE") .gt. 0) then 
           done = .true.
           if(.not.found) then
             print*, 'Error, no information about Photon PDF found'
           endif
           return
        endif
 10   done=.true.
...
c**********************************************************************
      ...
      open(29,file='../Cards/proc_card.dat',status='unknown')

      do while (.not.done)

      read(29,'(a)',end=10) ,buff
