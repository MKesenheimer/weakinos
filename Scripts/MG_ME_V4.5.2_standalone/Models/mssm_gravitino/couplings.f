...
c
     ...
     &                  mu, a_top, a_bot, a_tau
      ...
      double complex gfrs,gfrv !for gravitino
      double precision Mp !for gravitino
...
c
      ...
      mgro= lowmass(99) !gravitino mass
c
...
c   Higgs widths not in the standard version - should overwrite w/ Hdecay
         ...
         wgro= lowwidth(99) !gravitino width
...
c************************************            
c Gravitino couplings
c************************************
      Mp = 1.22089d19/dsqrt(8d0*pi) ! reduced Planck mass (\sim 2.4x10^18)

c FRS couplings
      GFRS = dcmplx(1d0/dsqrt(2d0)/Mp,zero)

      GFRSL(1) = GFRS
      GFRSL(2) = zero
      GFRSR(1) = zero
      GFRSR(2) =-GFRS
      GFRB1(1) = r_b(1,1) *GFRSL(1)
      GFRB1(2) = r_b(1,2) *GFRSR(2)
      GFRB2(1) = r_b(2,1) *GFRSL(1)
      GFRB2(2) = r_b(2,2) *GFRSR(2)
      GFRT1(1) = r_t(1,1) *GFRSL(1)
      GFRT1(2) = r_t(1,2) *GFRSR(2)
      GFRT2(1) = r_t(2,1) *GFRSL(1)
      GFRT2(2) = r_t(2,2) *GFRSR(2)
      GFRL1(1) = r_l(1,1) *GFRSL(1)
      GFRL1(2) = r_l(1,2) *GFRSR(2)
      GFRL2(1) = r_l(2,1) *GFRSL(1)
      GFRL2(2) = r_l(2,2) *GFRSR(2)

      GN1RH1(1)= (ca*bw(1,4)-sa*bw(1,3))/rt2 *GFRSL(1)
      GN1RH1(2)= zero
      GN2RH1(1)= (ca*bw(2,4)-sa*bw(2,3))/rt2 *GFRSL(1)
      GN2RH1(2)= zero
      GN3RH1(1)= (ca*bw(3,4)-sa*bw(3,3))/rt2 *GFRSL(1)
      GN3RH1(2)= zero
      GN4RH1(1)= (ca*bw(4,4)-sa*bw(4,3))/rt2 *GFRSL(1)
      GN4RH1(2)= zero

      GN1RH2(1)= (sa*bw(1,4)+ca*bw(1,3))/rt2 *GFRSL(1)
      GN1RH2(2)= zero
      GN2RH2(1)= (sa*bw(2,4)+ca*bw(2,3))/rt2 *GFRSL(1)
      GN2RH2(2)= zero
      GN3RH2(1)= (sa*bw(3,4)+ca*bw(3,3))/rt2 *GFRSL(1)
      GN3RH2(2)= zero
      GN4RH2(1)= (sa*bw(4,4)+ca*bw(4,3))/rt2 *GFRSL(1)
      GN4RH2(2)= zero

      GN1RH3(1)= (cb*bw(1,4)+sb*bw(1,3))/rt2 *GFRSL(1) *i_unit
      GN1RH3(2)= zero
      GN2RH3(1)= (cb*bw(2,4)+sb*bw(2,3))/rt2 *GFRSL(1) *i_unit
      GN2RH3(2)= zero
      GN3RH3(1)= (cb*bw(3,4)+sb*bw(3,3))/rt2 *GFRSL(1) *i_unit
      GN3RH3(2)= zero
      GN4RH3(1)= (cb*bw(4,4)+sb*bw(4,3))/rt2 *GFRSL(1) *i_unit
      GN4RH3(2)= zero

      GX1RH(1) = cb*vv(1,2) *GFRSL(1) 
      GX1RH(2) = sb*uu(1,2) *GFRSR(2)
      GX2RH(1) = cb*vv(2,2) *GFRSL(1) 
      GX2RH(2) = sb*uu(2,2) *GFRSR(2)

c FRV couplings
      GFRV = dcmplx(1d0/4d0/Mp,zero)

      do i1=1,2 !left,right
 
      GGORG(i1) = GFRV
 
      GN1RA(i1) = ( cw*bw(1,1)+sw*bw(1,2)) *GFRV
      GN2RA(i1) = ( cw*bw(2,1)+sw*bw(2,2)) *GFRV
      GN3RA(i1) = ( cw*bw(3,1)+sw*bw(3,2)) *GFRV
      GN4RA(i1) = ( cw*bw(4,1)+sw*bw(4,2)) *GFRV

!   NOTE: The higgsino contribution is neglected.

      GN1RZ(i1) = (-sw*bw(1,1)+cw*bw(1,2)) *GFRV
      GN2RZ(i1) = (-sw*bw(2,1)+cw*bw(2,2)) *GFRV
      GN3RZ(i1) = (-sw*bw(3,1)+cw*bw(3,2)) *GFRV
      GN4RZ(i1) = (-sw*bw(4,1)+cw*bw(4,2)) *GFRV
...
c FRVS couplings
      ...
      GFRADL(i1) = GFRSL(i1) * GAD(i1)
      GFRADR(i1) = GFRSR(i1) * GAD(i1)
      GFRAUL(i1) = GFRSL(i1) * GAU(i1)
      GFRAUR(i1) = GFRSR(i1) * GAU(i1)
      GFRAB1(i1) = GFRB1(i1) * GAD(i1)
      GFRAB2(i1) = GFRB2(i1) * GAD(i1)
      GFRAT1(i1) = GFRT1(i1) * GAU(i1)
      GFRAT2(i1) = GFRT2(i1) * GAU(i1)

      GFRZDL(i1) = GFRSL(i1) * GZD(i1)
      GFRZDR(i1) = GFRSR(i1) * GZD(i1)
      GFRZUL(i1) = GFRSL(i1) * GZU(i1)
      GFRZUR(i1) = GFRSR(i1) * GZU(i1)
      GFRZB1(i1) = GFRB1(i1) * GZD(i1)
      GFRZB2(i1) = GFRB2(i1) * GZD(i1)
      GFRZT1(i1) = GFRT1(i1) * GZU(i1)
      GFRZT2(i1) = GFRT2(i1) * GZU(i1)

      GFRWSL(i1) = GFRSL(i1) * GWF(i1)
      GFRWB1(i1) = GFRB1(i1) * GWF(i1)
      GFRWB2(i1) = GFRB2(i1) * GWF(i1)
      GFRWT1(i1) = GFRT1(i1) * GWF(i1)
      GFRWT2(i1) = GFRT2(i1) * GWF(i1)

      GFRASL(i1) = GFRSL(i1) * GAL(i1)
      GFRASR(i1) = GFRSR(i1) * GAL(i1)
      GFRAL1(i1) = GFRL1(i1) * GAL(i1)
      GFRAL2(i1) = GFRL2(i1) * GAL(i1)

      GFRZSL(i1) = GFRSL(i1) * GZL(i1)
      GFRZSR(i1) = GFRSR(i1) * GZL(i1)
      GFRZL1(i1) = GFRL1(i1) * GZL(i1)
      GFRZL2(i1) = GFRL2(i1) * GZL(i1)
      GFRZSN(i1) = GFRSL(i1) * GZN(i1)

      GN1RZH1(i1) = GN1RH1(i1) * GZN(i1)
      GN2RZH1(i1) = GN2RH1(i1) * GZN(i1)
      GN3RZH1(i1) = GN3RH1(i1) * GZN(i1)
      GN4RZH1(i1) = GN4RH1(i1) * GZN(i1)

      GN1RZH2(i1) = GN1RH2(i1) * GZN(i1)
      GN2RZH2(i1) = GN2RH2(i1) * GZN(i1)
      GN3RZH2(i1) = GN3RH2(i1) * GZN(i1)
      GN4RZH2(i1) = GN4RH2(i1) * GZN(i1)

      GN1RZH3(i1) = GN1RH3(i1) * GZN(i1)
      GN2RZH3(i1) = GN2RH3(i1) * GZN(i1)
      GN3RZH3(i1) = GN3RH3(i1) * GZN(i1)
      GN4RZH3(i1) = GN4RH3(i1) * GZN(i1)

      enddo

      GX1RWH1(1) = ca*uu(1,2)/rt2 * GWF(1)
      GX1RWH1(2) = ca*vv(1,2)/rt2 * GWF(2)
      GX1RWH2(1) =-sa*uu(1,2)/rt2 * GWF(1)
      GX1RWH2(2) =-sa*vv(1,2)/rt2 * GWF(2)
      GX1RWH3(1) = cb*uu(1,2)/rt2 * GWF(1) * i_unit
      GX1RWH3(2) = cb*vv(1,2)/rt2 * GWF(2) * i_unit
      GX2RWH1(1) = ca*uu(2,2)/rt2 * GWF(1)
      GX2RWH1(2) = ca*vv(2,2)/rt2 * GWF(2)
      GX2RWH2(1) =-sa*uu(2,2)/rt2 * GWF(1)
      GX2RWH2(2) =-sa*vv(2,2)/rt2 * GWF(2)
      GX2RWH3(1) = cb*uu(2,2)/rt2 * GWF(1) * i_unit
      GX2RWH3(2) = cb*vv(2,2)/rt2 * GWF(2) * i_unit

      do i1=1,2 !left,right

      GX1RAH(i1) = GX1RH(i1) * GAL(i1)
      GX2RAH(i1) = GX2RH(i1) * GAL(i1)

      GX1RZH(i1) = GX1RH(i1) * GZL(i1)
      GX2RZH(i1) = GX2RH(i1) * GZL(i1)

      GN1RWH(i1) = sb*bw(1,3) * GWF(i1)
      GN2RWH(i1) = sb*bw(2,3) * GWF(i1)   
      GN3RWH(i1) = sb*bw(3,3) * GWF(i1)
      GN4RWH(i1) = sb*bw(4,3) * GWF(i1)    
      GRN1WH(i1) = cb*bw(1,4) * GWF(i1)
      GRN2WH(i1) = cb*bw(2,4) * GWF(i1)   
      GRN3WH(i1) = cb*bw(3,4) * GWF(i1)
      GRN4WH(i1) = cb*bw(4,4) * GWF(i1)    
...
c FRVV couplings
      ...
      GN1RWW(i1) = gx * bw(1,3) * GFRV
      GN2RWW(i1) = gx * bw(2,3) * GFRV
      GN3RWW(i1) = gx * bw(3,3) * GFRV
      GN4RWW(i1) = gx * bw(4,3) * GFRV

      enddo

      GX1RWA(1) = gx * sw * uu(1,1) * GFRV
      GX1RWA(2) = gx * sw * vv(1,1) * GFRV
      GX2RWA(1) = gx * sw * uu(2,1) * GFRV
      GX2RWA(2) = gx * sw * vv(2,1) * GFRV

      GX1RWZ(1) = gx * cw * uu(1,1) * GFRV
      GX1RWZ(2) = gx * cw * vv(1,1) * GFRV
      GX2RWZ(1) = gx * cw * uu(2,1) * GFRV
      GX2RWZ(2) = gx * cw * vv(2,1) * GFRV
...
c FRVS and FRVV couplings

      do i1=1,2

      GFRGSL(i1) = GFRSL(i1) * GGi(i1)
      GFRGSR(i1) = GFRSR(i1) * GGi(i1)
      GFRGB1(i1) = GFRB1(i1) * GGi(i1)
      GFRGB2(i1) = GFRB2(i1) * GGi(i1)
      GFRGT1(i1) = GFRT1(i1) * GGi(i1)
      GFRGT2(i1) = GFRT2(i1) * GGi(i1)

      GGORGG(i1) = GGORG(i1) * Gs
