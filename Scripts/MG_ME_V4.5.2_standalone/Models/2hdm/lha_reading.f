...
c
      ...
      do while(.true.) !run over reference file
        ...
        read(ref_file,'(a132)',err=99,end=99) buffer        
       ...
       if(temp .eq. blockname) then
		...
		do while(.not. islast)
			...
			if (first_ref .ne. first_line) exit
		end do
	end if
 98   end do
 99   close(ref_file)
...
c-----
c
c     first check if the card is in the main directory
c
      tempname=filename
      fine=index(tempname,' ')
      if(fine.eq.0) fine=len(tempname)
      tempname=tempname(1:fine-1)
...
c
 ...
 30      tempname='../'//tempname
         if (i.eq.5)then
            write(*,*) 'Warning: file ',tempname(27:),' is not found'
            stop
         endif
      enddo
