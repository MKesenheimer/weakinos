...
c Higgs coupling:
	...
	gphph(2) = dcmplx( Zero, Zero )
      ...
      gh(1) = dcmplx( scalarf*dsqrt(4*Pi*0.120)**2/4d0/PI/(3d0*PI*V)*series_t, Zero)
      gh(2) = dcmplx( axialf *dsqrt(4*Pi*0.120)**2/4d0/PI/(2d0*PI*V)*series_p, Zero)

c Pseudo-scalar Higgs coupling:
      ga(1) = dcmplx( Zero                  , Zero)
      ga(2) = dcmplx( dsqrt(4*Pi*0.120)**2/4d0/PI/(2d0*PI*V)*series_p, Zero)
