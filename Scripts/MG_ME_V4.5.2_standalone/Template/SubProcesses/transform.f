...
c                  print*, msq_ij,msq_k,dot(pij,pij),(2d0*dot(p(0,ip),p(0,jp)))
 ...
 24         continue
      ...
      subroutine transformqed(p,q,ip,jp,kp,ijp,kpp,mass_i,mass_j,mass_k)
************************************************************************
* The equivalent subroutine as transform, but for QED dipoles
************************************************************************
      implicit none 
      include 'nexternal.inc'
      double precision p(0:3,nexternal),q(0:3,nexternal),x,omx,y,omy,
     ...
     . kDp(nincoming+1:nexternal),ksDp(nincoming+1:nexternal)
      ...
      double precision mu_k,mu_ij,qtot(0:3),qsq,pij(0:3),lambda_tr,msq_ij
      double precision msq_k,msq_i,msq_j,porig(0:3,nexternal),beta,gamma
      double precision pia(0:3),piasq,pijsq,pab(0:3),sab,pabsq
      external dot,lambda_tr
      double precision one, two
      parameter (one=1d0,two=2d0)
      logical firsttime
...
c --- initial-initial
        do nu=0,3
         pab(nu)=p(nu,ip)+p(nu,kp)-p(nu,jp)
        enddo
        sab=msq_i+msq_k+2d0*dot(p(0,ip),p(0,kp))
        pabsq=dot(pab,pab)

         x=one-(dotij+dotjk)/dotik

         do nu=0,3
            q(nu,ijp)= sqrt(lambda_tr(pabsq,msq_i,msq_k)
     &                 /lambda_tr(sab,msq_i,msq_k))*(p(nu,ip)-dot(p(0,ip),p(0,kp))/
     &                 msq_k*p(nu,kp))+(pabsq-msq_i-msq_k)/(2d0*msq_k)*p(nu,kp)
            q(nu,kpp)=  p(nu,kp)
            k(nu)    =  p(nu,ip)+p(nu,kp)-p(nu,jp)
            kt(nu)   =  q(nu,ijp)+p(nu,kp)
            ks(nu)   =  k(nu)+kt(nu)
...
c  final-initial
         ...
         do j=1,nexternal
            do nu=0,3
               if (j.eq.inp) then
                  q(nu,ipart)=sqrt(lambda_tr(piasq,msq_k,msq_i)/
     &                  lambda_tr(pijsq,piasq,msq_k))*(p(nu,kp)
     &                  -dot(pia,p(0,kp))/piasq*pia(nu))+(piasq-msq_k+msq_i)
     &                   /(2d0*piasq)*pia(nu)- pia(nu) 
                  if ((ipart.ne.ijp).and.(ipart.ne.kpp))then
                     write(*,*)'ERROR, problem with momentum mapping fi'
                  endif
               elseif (j.eq.jp) then
...
c initial-final
         ...
         do j=1,nexternal
            do nu=0,3
               if (j.eq.inp) then
                  q(nu,ipart)=sqrt(lambda_tr(piasq,msq_i,msq_k)/
     &                  lambda_tr(pijsq,piasq,msq_i))*(p(nu,ip)
     &                  -dot(pia,p(0,ip))/piasq*pia(nu))+(piasq-msq_i+msq_k)
     &                   /(2d0*piasq)*pia(nu) - pia(nu)
                  if ((ipart.ne.ijp).and.(ipart.ne.kpp))then
                     write(*,*)'ERROR, problem with momentum mapping if'
                  endif
               elseif (j.eq.jp) then
                  goto 231
               elseif (j.eq.fip) then
     ...
     &                   /(2d0*piasq)*pia(nu)
         ...
         enddo
            if(ipart.gt.nexternal)then
               write(*,*) 'ERROR, ipart .ge. nexternal in if'
            endif
 231         continue
...
c---  final-final
       ...
       qsq=dot(qtot,qtot)
         do j=1,nexternal
            do nu=0,3
               if (j.eq.ip) then
                 q(nu,ipart)=qtot(nu)-(sqrt(lambda_tr(qsq,msq_i,msq_k))
     &                        /sqrt(lambda_tr(dot(pij,pij),qsq,msq_k))
     &                        *(p(nu,kp)-dot(qtot,p(0,kp))/qsq*qtot(nu))
     &                        +(qsq+msq_k-msq_i)/two/qsq*qtot(nu))
                  if (ipart.ne.ijp)then
                     write(*,*)'ERROR, problem with momentum mapping ff'
                  endif
               elseif (j.eq.jp) then
               ...
               elseif (j.eq.kp) then
c                  print*, msq_ij,msq_k,dot(pij,pij),(2d0*dot(p(0,ip),p(0,jp)))
                  q(nu,ipart)=sqrt(lambda_tr(qsq,msq_i,msq_k))
     &                        /sqrt(lambda_tr(dot(pij,pij),qsq,msq_k))
     &                        *(p(nu,kp)-dot(qtot,p(0,kp))/qsq*qtot(nu))
     &                        +(qsq+msq_k-msq_i)/two/qsq*qtot(nu)
                  if (ipart.ne.kpp)then
 ...
 24         continue
         enddo
c         print*, dot(q(0,ip),q(0,ip)),dot(q(0,kp),q(0,kp))!,dot(q(0,3),q(0,3)),dot(q(0,4),q(0,4))
c         do j=1,nexternal
c          do nu=0,3
c             p(nu,j)=porig(nu,j)
c          enddo
