      ...
      integer isym(nexternal,99), jsym, maxcolor
...
c      print *,'Chose color flow ',ic
      ...
      do i=1,nexternal
         if(ic.gt.0) then
            ...
            if (icolup(1,i,ic).gt.maxcolor) maxcolor=icolup(1,i,ic)
            if (icolup(2,i,ic).gt.maxcolor) maxcolor=icolup(2,i,ic)
         else
...
c       Fist set "safe" color info
     ...
     $       icolalt(2,ida(1))-icolalt(2,ida(2)).eq.0) then ! color singlet
            icolalt(1,i) = 0
            icolalt(2,i) = 0            
          elseif(icolalt(1,ida(1))-icolalt(2,ida(2)).eq.0) then ! color octet
            icolalt(1,i) = icolalt(1,ida(2))
            icolalt(2,i) = icolalt(2,ida(1))
          else if(icolalt(1,ida(2))-icolalt(2,ida(1)).eq.0) then ! color octet
...
c         Unsupported color assignment for propagator - set color to 0
            icolalt(1,i) = 0
            icolalt(2,i) = 0
          endif
