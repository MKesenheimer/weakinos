...
class Multi_Collector(dict):
    ...
    def define_fail(self, name):
        ...
        if minimal_precision:
            for result in self[name].values():
                if result['value'] and result['error']/result['value'] > minimal_precision:
                    failed.append([result['card'],result['events']])
...
class Collect_All_MW(Multi_Collector):
    ...
    def collect_dir(self, path):
        ...
        dir_name = [name for name in path.split('/') if name.startswith('MW_')][-1]
