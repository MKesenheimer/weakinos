!WTARO!: to add a vertex with a spin-3/2 particle.
...
c************************** WTARO start ********************************
      subroutine addFRS1(ipart,prtcl1,coup,info,icoup,flagC)
c***********************************************************************
c     Sets up the IORS vertex with all the permutations
c     info is the color info for the IORS configuration
c     icoup is the coupling constant
c***********************************************************************
      implicit none
...
! Constants
      ...
      include 'params.inc'
      character*(*) IORS_head                          
      parameter    (IORS_head='IORSXX(W(1,1???),W(1,2???),W(1,3???),')
      character*(*) IORS_tail                          
      parameter    (IORS_tail=',AMP(????))')                         
      character*(*) HIOR_head                   
      parameter    (HIOR_head='HIORXX(W(1,1???),W(1,2???),')
      character*(*) HIOR_tail                   
      parameter    (HIOR_tail=',W(1,3???))')
      character*(*) FSOR_head                
      parameter    (FSOR_head='FSORXX(W(1,2???),W(1,3???),')
      character*(*) FSOR_tail
      parameter    (FSOR_tail=',W(1,1???))')
      character*(*) IROS_head                          
      parameter    (IROS_head='IROSXX(W(1,1???),W(1,2???),W(1,3???),')
      character*(*) IROS_tail                          
      parameter    (IROS_tail=',AMP(????))')                         
      character*(*) HIRO_head                   
      parameter    (HIRO_head='HIROXX(W(1,1???),W(1,2???),')
      character*(*) HIRO_tail                   
      parameter    (HIRO_tail=',W(1,3???))')
      character*(*) FSIR_head                
      parameter    (FSIR_head='FSIRXX(W(1,1???),W(1,3???),')
      character*(*) FSIR_tail
      parameter    (FSIR_tail=',W(1,2???))')
...
! Local
      ...
      integer ipart2(0:4),i
      character*(max_string) buff
      character*8 fmass,fwidth,ymass,ywidth,smass,swidth,rmass,rwidth
ccccccccc WTARO cccccccccc
	  integer flagC
	  integer porder
...
c-----
      ...
      do i=0,4
         ipart2(i)=ipart(i)                        
      enddo
      buff = IORS_head//coup//IORS_tail
      call add3vertex(ipart2,buff,info,icoup)      !Send the IORS version

      do i=0,4
         ipart2(i)=ipart(i)
      enddo
      ipart2(3)=1                                  !3rd particle needed S
      ipart2(4)=inverse(ipart(3))                  !get antiparticle
      smass  = str(2,ipart(3))                     !Mass of Scalar Boson
      swidth = str(3,ipart(3))                     !Width of Scalar Boson
      buff = HIOR_head//coup//','
     $//smass//','//swidth//HIOR_tail
      call add3vertex(ipart2,buff,info,icoup)      !Send the HIORXX version
      ...
      fmass  = str(2,ipart(1))                     !Mass of Fermion
      fwidth = str(3,ipart(1))                     !Width of fermion
      buff = FSOR_head//coup//','//fmass//','//fwidth//FSOR_tail
      call add3vertex(ipart2,buff,info,icoup)      !Send the FSORXX version

      else
          do i=0,4
         ipart2(i)=ipart(i)                        
      enddo
      buff = IROS_head//coup//IROS_tail
      call add3vertex(ipart2,buff,info,icoup)      !Send the IROS version
      ...
      swidth = str(3,ipart(3))                     !Width of Scalar Boson
      buff = HIRO_head//coup//','
     $//smass//','//swidth//HIRO_tail
      call add3vertex(ipart2,buff,info,icoup)      !Send the HIROXX version

      do i=0,4
         ipart2(i)=ipart(i)
      enddo
      ipart2(2) = 1                                !1st particle needed I
      ipart2(4) = inverse(ipart(2))                !get antiparticle
      fmass  = str(2,ipart(2))                     !Mass of Fermion
      fwidth = str(3,ipart(2))                     !Width of fermion
      buff = FSIR_head//coup//','//fmass//','//fwidth//FSIR_tail
      call add3vertex(ipart2,buff,info,icoup)      !Send the FSIRXX version
      ...
      subroutine addFRV1(ipart,prtcl1,coup,info,icoup,flagC)
c**************************************************************************
c     Sets up the FRV vertex with all the permutations
c     info is the color info for the FRV configuration
c     icoup is the coupling constant
c*************************************************************************
      implicit none
...
! Constants
      ...
      include 'params.inc'
      character*(*) IORV_head                          
      parameter    (IORV_head='IORVXX(W(1,1???),W(1,2???),W(1,3???),')
      character*(*) IORV_tail                          
      parameter    (IORV_tail=',AMP(????))')                         
      character*(*) JIOR_head                   
      parameter    (JIOR_head='JIORXX(W(1,1???),W(1,2???),')
      character*(*) JIOR_tail                   
      parameter    (JIOR_tail=',W(1,3???))')
      character*(*) FVOR_head                
      parameter    (FVOR_head='FVORXX(W(1,2???),W(1,3???),')
      character*(*) FVOR_tail
      parameter    (FVOR_tail=',W(1,1???))')
      character*(*) IROV_head                          
      parameter    (IROV_head='IROVXX(W(1,1???),W(1,2???),W(1,3???),')
      character*(*) IROV_tail                          
      parameter    (IROV_tail=',AMP(????))')                         
      character*(*) JIRO_head                   
      parameter    (JIRO_head='JIROXX(W(1,1???),W(1,2???),')
      character*(*) JIRO_tail                   
      parameter    (JIRO_tail=',W(1,3???))')
      character*(*) FVIR_head                
      parameter    (FVIR_head='FVIRXX(W(1,1???),W(1,3???),')
      character*(*) FVIR_tail
      parameter    (FVIR_tail=',W(1,2???))')     
...
! Local
      ...
      integer ipart2(0:4),i
      character*(max_string) buff
      character*8 fmass,fwidth,vmass,vwidth,smass,swidth
      character*8 rmass,rwidth
ccccccccc WTARO cccccccccc
	  integer flagC
	  integer porder
cccccccccccccccccccccccccc
...
c-----
      ...
      do i=0,4
         ipart2(i)=ipart(i)                        
      enddo
      buff = IORV_head//coup//IORV_tail
      call add3vertex(ipart2,buff,info,icoup)      !Send the IORV version
      ...
      ipart2(3)=1                                  !3rd particle needed V
      ipart2(4)=inverse(ipart(3))                  !get antiparticle
      vmass  = str(2,ipart(3))                     !Mass of Scalar Boson
      vwidth = str(3,ipart(3))                     !Width of Scalar Boson
      buff = JIOR_head//coup//','
     $//vmass//','//vwidth//JIOR_tail
      call add3vertex(ipart2,buff,info,icoup)      !Send the JIORXX version
      ...
      fmass  = str(2,ipart(1))                     !Mass of Fermion
      fwidth = str(3,ipart(1))                     !Width of fermion
      buff = FVOR_head//coup//','//fmass//','//fwidth//FVOR_tail
      call add3vertex(ipart2,buff,info,icoup)      !Send the FVORXX version

      else
          do i=0,4
         ipart2(i)=ipart(i)                        
      enddo
      buff = IROV_head//coup//IROV_tail
      call add3vertex(ipart2,buff,info,icoup)      !Send the IROV version
      ...
      vwidth = str(3,ipart(3))                     !Width of Scalar Boson
      buff = JIRO_head//coup//','
     $//vmass//','//vwidth//JIRO_tail
      call add3vertex(ipart2,buff,info,icoup)      !Send the JIROXX version
      ...
      fmass  = str(2,ipart(2))                     !Mass of Fermion
      fwidth = str(3,ipart(2))                     !Width of fermion
      buff = FVIR_head//coup//','//fmass//','//fwidth//FVIR_tail
      call add3vertex(ipart2,buff,info,icoup)      !Send the FVIRXX version
