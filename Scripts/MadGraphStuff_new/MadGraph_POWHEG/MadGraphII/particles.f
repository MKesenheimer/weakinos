!WTARO!: to add a spin-3/2 particle.
...
c*******************WTARO***************************************************
      subroutine addVFermion(code,anticode,mass,width,name,
     $     icolor,ihel,iline,ireq)
c***************************************************************************
c     Adds Rarita-Schwinger Fermion both the particle and the antiparticle
...
c
      ...
      parameter (   buffa_tail=',NHEL(????),-1*IC(????),W(1,????))')
      character*(*) buffi_head               
      parameter (   buffi_head='IRXXXX(P(0,????),')
      character*(*) buffo_head                
      parameter (   buffo_head='ORXXXX(P(0,????),')
c
c     Arguments
c
      character*(*) code,anticode,mass,width,name
...
c
c     Now add charge conjugate fields for Dirac VFermions
c     uses the fact that particle numbers are added sequentially to add
c     the charge-conjugate field. I am defining this field to be a spinor
c     with the same quantum numbers, but with 'ficticious flow' in the opposite
c     direction. See Denner, Eck, Hahn, Kublbeck CERN-TH.6549/92
...
c            inverse(jline+3)  = jline+2            
         ...
         print*,'Error VFermion not entered properly',npart,code
      endif
      end      
c*******************WTARO***************************************************    
...
c      write(*,*) 'Writing to pmass'
      if (first_time.and..not.DipolSub.and..not.DipolSubQED) then
      ...
      do i=1,iline(0)
         if (first_time.and..not.onium.and..not.DipolSub.and..not.DipolSubQED) then
...
c     print*,'Sorry no helicity here',iposx(2,ipart)
      ...
      enddo
      if (first_time.and..not.DipolSub.and..not.DipolSubQED) close(unit=itnum)
...
c******************WTARO********************************
         if (info_p(2,ipart)          .eq. -6 .or.
     $       info_p(2,inverse(ipart)) .eq. -6)      then
            majarana=.true.
         endif
c******************WTARO********************************
      endif
