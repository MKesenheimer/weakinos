...
!-----------
      ...
      enddo
c$$$      open(unit=isubfile,file='mg.sym',status='unknown')
c$$$      write(isubfile,*) ident
c$$$      do j=1,jmatch(0,0)
c$$$         if (jmatch(j,0) .gt. 1) then
c$$$            write(isubfile,*) jmatch(j,0)
c$$$            do i=1,jmatch(j,0)
c$$$               write(isubfile,*) jmatch(i,j)
c$$$            enddo
c$$$         endif
c$$$      enddo
c$$$      close(isubfile)
...
c         write(isubfile,'(a$)') gname(1:iname),'.o '
      else
         open(unit=ln,status='unknown')
      endif
c$$$      if (dipol.le.nrc)then
c$$$         name='BORN'
c$$$         nlength=4
c$$$      else
         name = gname(1:iname)
...
c$$$      endif
     ...
     &     ' in file ',GNAME(1:iname)//'.f.'
      ...
      if (dipol.le.nrc .and. offdiag) then
         buff = 'COMPLEX*16 FUNCTION '
         jchar = 21
         ishift = nlength
         buff(jchar:jchar+ishift) = name(1:nlength)
         jchar = jchar+ishift
         ishift= 25
         buff(jchar:jchar+ishift) = '(P,NHEL,HELL,IC,BORNTILDE)'
         jchar =jchar+ishift
      else
...
! write comments
      ...
      write(ln,5) 'FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL-1)'
...
! write argument declarations
      ...
      write(ln,10) 'INTEGER NHEL(NEXTERNAL-1), IC(NEXTERNAL-1), HELL'
      if(dipol.le.nrc .and. offdiag) then
         write(ln,10) 'COMPLEX*16 BORNTILDE'
      endif
...
! write local declarations
      ...
      if(dipol.le.nrc .and. offdiag) write(ln,10) 'REAL*8 RTEMP'
...
cend RF
      if(dipol.le.nrc .and. offdiag)then
         write(ln,10) 'integer ihel'
         write(ln,10) 'COMPLEX*16 JAMPH(-1:1,NCOLOR)'
      endif
...
! write global declarations
      ...
      write(ln,10) 'common/to_amps_'//cpn//'/  amp2,       jamp2'

      write(92,10) 'integer max_bhel'
      write(92,*) '     parameter ( max_bhel =', ncomb,')'
      write(ln,10)'double complex saveamp(ngraphs,max_bhel)'
      write(ln,10)'common/to_saveamp_'//cpn//'/saveamp'
      write(92,10) 'logical calculatedBorn'
      write(92,10) 'integer skip'
      write(92,10) 'common/cBorn/calculatedBorn,skip'
 ...
 99   continue
      ...
      if(dipol.le.nrc .and. offdiag) then
         ...
         buff(nlength+1:nlength+14) = ' = (0D0,0D0)'
         write(ln,10) buff(1:nlength+14)
         write (ln,10) 'BORNTILDE = (0d0,0d0)'
         write (ln,10) 'do ihel=-1,1,2'
      endif
      write(ln,10) 'if (.not. calculatedBorn) then'

      i=0
      emit=info_ijk(dipol,1,3)
      do while (.true.)
         i=i+1
         read(91,'(a)',end=999) buff
         if (dipol.le.nrc .and. offdiag .and. i.eq.emit)then
            if (emit.ge.3) then
               write (ln,'(a22,i3,a18,i3,a6,i3,a2)')
     &              '      CALL VXXXXX(P(0,',emit,
     &              '),ZERO,ihel,+1*IC(',emit,'),W(1,',emit,'))'
            else
               write (ln,'(a22,i3,a18,i3,a6,i3,a2)')
     &              '      CALL VXXXXX(P(0,',emit,
     &              '),ZERO,ihel,-1*IC(',emit,'),W(1,',emit,'))'
 ...
 999  continue
      ...
      if (.not.offdiag .or. dipol.gt.nrc) then
         write(ln,10) 'do i=1,ngraphs'
         write(ln,10) '   saveamp(i,hell)=amp(i)'
         write(ln,10) 'enddo'
      elseif(offdiag .and. dipol.le.nrc) then
         write(ln,10) 'do i=1,ngraphs'
         write(ln,10) '   if(ihel.eq.-1)then'
         write(ln,10) '      saveamp(i,hell)=amp(i)'
         write(ln,10) '   elseif(ihel.eq.1)then'
         write(ln,10) '      saveamp(i,hell+skip)=amp(i)'
         write(ln,10) '   else'
         write(ln,10) '      write(*,*) "ERROR #1 in born.f"'
         write(ln,10) '      stop'
         write(ln,10) '   endif'
         write(ln,10) 'enddo'
      endif
      write (ln,10) 'elseif (calculatedBorn) then'
      if (.not.offdiag .or. dipol.gt.nrc) then
         write(ln,10) 'do i=1,ngraphs'
         write(ln,10) '   amp(i)=saveamp(i,hell)'
         write(ln,10) 'enddo'
      elseif(offdiag .and. dipol.le.nrc) then
         write(ln,10) 'do i=1,ngraphs'
         write(ln,10) '   if(ihel.eq.-1)then'
         write(ln,10) '      amp(i)=saveamp(i,hell)'
         write(ln,10) '   elseif(ihel.eq.1)then'
         write(ln,10) '      amp(i)=saveamp(i,hell+skip)'
         write(ln,10) '   else'
         write(ln,10) '      write(*,*) "ERROR #1 in born.f"'
         write(ln,10) '      stop'
         write(ln,10) '   endif'
...
c     $                 ,den(j)/den(i),'*AMP(',j,')'
      ...
      if (dipol.gt.nrc .or. .not.offdiag) then
         buff(1:nlength) = name(1:nlength)
      ...
      write(ln,10) '    ENDDO'
      if (dipol.le.nrc .and. offdiag) then
         write(ln,10) '    RTEMP=DBLE(ZTEMP*DCONJG(JAMP(I))/DENOM(I))'
         buff(1:7) = '       '
         buff(8:nlength+7) = name(1:nlength)
         buff(nlength+8:nlength+8) = '='
         buff(nlength+9:2*nlength+9) = name(1:nlength)
         write(ln,10) '    if (ihel.eq.-1) then'
         buff(2*nlength+10:2*nlength+15) = '+RTEMP'
         write(ln,10) buff(1:2*nlength+15)
         write(ln,10) '    elseif (ihel.eq.1) then'
         buff(2*nlength+10:2*nlength+25) = '+(0d0,1d0)*RTEMP'
         write(ln,10) buff(1:2*nlength+25)
         write(ln,10) '    endif'
      else
         buff(1:4) = '    '
     ...
     .        '+ZTEMP*DCONJG(JAMP(I))/DENOM(I)'
      ...
      if(dipol.le.nrc .and. offdiag) then
         write(ln,10) '    Jamph(ihel,i)=Jamp(i)'
         write(ln,10) 'Enddo'
         write(ln,10) ' '
      endif
      write(ln,10) 'Enddo'
      ...
      write(ln,10) '    ZTEMP = (0.D0,0.D0)'
      write(ln,10) '    DO J = 1, NCOLOR'
      write(ln,10) '        ZTEMP = ZTEMP + CF(J,I)*JAMPH(1,J)'
      write(ln,10) '    ENDDO'
      write(ln,10) '    BORNTILDE = BORNTILDE + ZTEMP*DCONJG('//
     &     'JAMPH(-1,I))/DENOM(I)'
      write(ln,10) 'ENDDO'
      endif
...
c if gluon splits we need off-diagonal (= Q terms) for AP splitting func.
      if ( info_ijk(dipol,2,3).eq.8.and.
     &     mass(info_ijk(dipol,1,3)).eq.'ZERO') then
         offdiag=.true.
      else
         offdiag=.false.
      endif
...
c$$$            if(FKSsub) iden(imatch(i)) = isymn1*initc*jhel
...
c$$$      if (dipol.le.nrc) then
c$$$         call sethel(nhel,step,nexternal,ihel)
c$$$         totalhelicities=ihel
c$$$         open (unit=61,file='born_nhel'//number//'.inc'
c$$$     &           ,status='unknown')
c$$$         write(61,'(6x,a)')     'integer    max_bhel,max'      
c$$$         write(61,'(6x,a,i5,a)')'parameter (max_bhel= ',
c$$$     &        totalhelicities,')'
c$$$         close(61)
c$$$      endif
c$$$c Ignore the sum over the helicities of the mother
c$$$      if (dipol.le.nrc .and. offdiag)then
c$$$         step(info_ijk(dipol,1,3))=3
c$$$      endif
...
c$$$      if (dipol.le.nrc)then
c$$$         ishift=4
c$$$         buff(jchar:jchar+ishift) = 'BORN'
c$$$         jchar = jchar+ishift
c$$$      else
         ishift = iname
         buff(jchar:jchar+ishift) = name(1:nlength)
         jchar = jchar+ishift
c$$$      endif
      ...
      if (dipol.le.nrc .and. offdiag) then
         jchar=jchar-4
         FNAME= buff(13:jchar)
         FNAME(JCHAR-12:JCHAR-12+27)=',NHEL(1,IHEL),IHEL,JC(1),T1)'
         FLEN = jchar+27
      else
         jchar=jchar-4
         FNAME= buff(13:jchar)
...
! write comments
      ...
      write(92,5) 'FOR THE POINT IN PHASE SPACE P(0:3,NEXTERNAL-1)'
      write(92,5) ' '
      ...
      do i=1,ncross
         ...
         ichar = 17
          buff= 'BORN AMPLITUDE IS '
          ichar = 19
         do j=1,2
            call part_string(inverse(iline(ipart(j,i))),str1,nlen)
            part_type(dipol,j)=str1
            if(str1(2:3).eq.'~') then
             part_type(dipol,j)(2:4)='bar'
            endif            
...
c      write(buff(43:45),'(i3)') ncross
      ...
      write(92,5) ' '
      if (dipol.le.nrc)then
         write(92,10) 'REAL*8 P1(0:3,NEXTERNAL-1)'
         write(92,10) 'COMPLEX*16 ANS(NCROSS*2)'
      else
         write(92,10) 'REAL*8 P1(0:3,NEXTERNAL-1),ANS(NCROSS)'
      endif

      write(92,5) ' '
      write(92,5) 'LOCAL VARIABLES '
      write(92,5) ' '
      write(92,10) 'REAL*8 P(0:3,NEXTERNAL-1)'
      write(92,10) 'INTEGER NHEL(NEXTERNAL-1,NCOMB),NTRY'
      if(dipol.le.nrc .and. offdiag) then
         write(92,10) 'COMPLEX*16 T,T1'
         buff = 'COMPLEX*16 '
         buff(12:11+nlength)=name(1:nlength)
         write(92,10) buff(1:11+nlength)
      else
      ...
      write(92,10) 'INTEGER IHEL,IDEN(NCROSS),IC(NEXTERNAL-1,NCROSS)'
      write(92,10) 'INTEGER IPROC,JC(NEXTERNAL-1), I,L,K'
      ...
      write(92,10) 'common/to_amps_'//cpn//'/  amp2,       jamp2'
      write(92,10)
      if (standalone) write(92,10)'integer j,jj'
      write(92,10) 'integer max_bhel'
      write(92,*) '     parameter ( max_bhel =', ihel,')'
      write(92,10) 'double complex saveamp(ngraphs,max_bhel)'
      write(92,10) 'common/to_saveamp_'//cpn//'/saveamp'
      write(92,10) 'double precision savemom(nexternal-1,2)'
      write(92,10) 'common/to_savemom_'//cpn//'/savemom'
      write(92,10)
      if (.not.standalone) then
      ...
      write(92,10) 'DATA xtry, xrej /0,0/'
      write(92,10) 'DATA warned, isum_hel/.false.,0/'
      write(92,10) 'DATA multi_channel/.true./'
      write(92,10) 'SAVE yfrac'

c$$$      buff = 'DATA NGRAPHS /'
c$$$      write(buff(16:24),'(i4,a)') ngraphs,'/'
c$$$      write(92,10) buff(1:30)
      endif
...
c      if (i .le. ncross) then
      ...
      write(92,10) 'integer skip'
      write(92,10) 'common/cBorn/calculatedBorn,skip'
c$$$      write(92,10) 'logical testhel'
c$$$      write(92,10) 'external testhel'
      ...
      if (dipol.le.nrc .and. offdiag)then
c$$$         write(92,10)'if (ntry.lt.2) then'
         emit =info_ijk(dipol,1,3)
         write(92,10)'skip=1'
         if (emit.le.9) then
            write(92,10)
     &           'do while(nhel('//char(48+emit)//',skip).ne.1)'
         else
            write(92,10) 'do while(nhel('//char(48+emit/10)
     &           //char(48+mod(emit,10))//',skip).ne.1)'
         endif
         write(92,10)'   skip=skip+1'
         write(92,10)'enddo'
         write(92,10)'skip=skip-1'
c$$$         write(92,10)'endif'
      endif
...
c$$$      write(92,10) 'CALL SWITCHMOM(P1,P,IC(1,IPROC),JC,NEXTERNAL-1)'
c
...
c
      if (nincoming .gt. 0) then
         write(92,10) 'DO IHEL=1,NEXTERNAL-1'
      ...
      write(92,10) 'ENDDO'
      write(92,10) 'if (calculatedBorn) then'
      write(92,10) '   do j=1,nexternal-1'
      write(92,10) '      if (savemom(j,1).ne.p1(0,j) .or.'
      write(92,12) '          savemom(j,2).ne.p1(3,j)) then'
      write(92,10) '         calculatedBorn=.false.'
      write(92,5) '         write (*,*) "momenta not the same in Born"'
      write(92,10) '      endif'
      write(92,10) '   enddo'
      write(92,10) 'endif'
      write(92,10) 'if (.not.calculatedBorn) then'
      write(92,10) '   do j=1,nexternal-1'
      write(92,10) '      savemom(j,1)=p1(0,j)'
      write(92,10) '      savemom(j,2)=p1(3,j)'
      write(92,10) '   enddo'
      write(92,10) '   do j=1,max_bhel'
      write(92,10) '      do jj=1,ngraphs'
      write(92,10) '         saveamp(jj,j)=(0d0,0d0)'
      write(92,10) '      enddo'
      write(92,10) '   enddo'
      write(92,10) 'endif'

      WRITE(92,10) 'ANS(IPROC) = 0D0'
      if (dipol.le.nrc .and. offdiag) write(92,10) 'ANS(IPROC+1) = 0D0'

c$$$      if (.not.standalone)then
c$$$         write(92,10)
c$$$     &        'write(hel_buff(1),''(16i5)'') (0,i=1,nexternal-1)'
c$$$         write(92,10) 'IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 2) THEN'
c$$$         if (dipol.le.nrc) then
c$$$            write(92,11)'   hel_fac=1d0'
c$$$         endif
c$$$      else
c$$$         write(92,10) 'IF (GET_HEL .EQ. 0 .OR. NTRY .LT. 2) THEN'
c$$$      endif
      

      write(92,11) 'DO IHEL=1,NCOMB'
c$$$      write(92,11) '  if(testhel(nexternal-1,nhel(1,ihel),ihel)) then'
      if (dipol.le.nrc .and. offdiag)then
         emit =info_ijk(dipol,1,3)
         write(92,11)
     &        '   IF ((GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2).and.'
         if (emit.le.9) then
            write(92,12) '   nhel('//char(48+emit)//',ihel).eq.-1) THEN'
         else
            write(92,12) '   nhel('//char(48+emit/10)
     &           //char(48+mod(emit,10))//',ihel).eq.-1) THEN'
         endif
      else
...
c$$$      if (.not.standalone)then
c$$$      write(92,13) ' DO JJ=1,nincoming'
c$$$      write(92,13) '   IF(POL(JJ).NE.1d0.AND.'
c$$$      write(92,14) '      NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN'
c$$$      write(92,13) '     T=T*ABS(POL(JJ))'
c$$$      if (dipol.le.nrc .and. offdiag)
c$$$     &     write(92,13) '     T1=T1*ABS(POL(JJ))'
c$$$      write(92,13) '   ELSE IF(POL(JJ).NE.1d0)THEN'
c$$$      write(92,13) '     T=T*(2d0-ABS(POL(JJ)))'
c$$$      if (dipol.le.nrc .and. offdiag)
c$$$     &     write(92,13) '     T1=T1*(2d0-ABS(POL(JJ)))'
c$$$      write(92,13) '   ENDIF'
c$$$      write(92,13) ' ENDDO'
c$$$      endif
      write(92,11) '     ANS(IPROC)=ANS(IPROC)+T'
      if (dipol.le.nrc .and. offdiag)
     &     write(92,11) '     ANS(IPROC+1)=ANS(IPROC+1)+T1'
      if (dipol.le.nrc .and. offdiag)then
         write(92,13)' IF ( (T .NE. (0D0,0D0) .OR.'
     &        //'T1 .NE. (0d0,0D0)) .AND.'
         write(92,14)'         .NOT. GOODHEL(IHEL,IPROC)) THEN'
      else
         write(92,13)' IF (T .NE. 0D0 .AND. .NOT. '//
...
c$$$      if (.not.standalone)then
c$$$      WRITE(92,11) 'JHEL = 1'
c$$$      WRITE(92,11) 'ISUM_HEL=MIN(ISUM_HEL,NGOOD)'
c$$$      endif

c$$$      WRITE(92,10) 'ELSE              !RANDOM HELICITY'
c$$$      if (.not.standalone) then
c$$$         WRITE(92,11) 'DO J=1,ISUM_HEL'
c$$$      endif
c$$$
c$$$      if (.not.standalone) then
c$$$         WRITE(92,11) '    HWGT = REAL(NGOOD)/REAL(ISUM_HEL)'
c$$$         if (dipol.le.nrc) then
c$$$            write(92,11) '    hel_fac=HWGT'
c$$$         endif
c$$$      else
c$$$         WRITE(92,11) '    HWGT = REAL(NGOOD)'
c$$$      endif
c$$$         
c$$$      if (.not.standalone) then
c$$$         WRITE(92,11) '    if (get_hel.eq.0) then'
c$$$         WRITE(92,11) '    JHEL=JHEL+1'
c$$$         WRITE(92,11) '    IF (JHEL .GT. NGOOD) JHEL=1'
c$$$         WRITE(92,11) '       IHEL = IGOOD(JHEL)'
c$$$         WRITE(92,11) '       get_hel=ihel'
c$$$         WRITE(92,11) '    else'
c$$$         WRITE(92,11) '       ihel=get_hel'
c$$$         WRITE(92,11) '    endif'
c$$$         WRITE(92,11) '    IF(GOODHEL(IHEL,IPROC)) THEN'         
c$$$      else
c$$$         WRITE(92,11) '    ihel=get_hel'
c$$$      endif
c$$$         
c$$$      buff(1:8) = '        '
c$$$      buff(8:9)= 'T='
c$$$      buff(10:10+flen-1) = fname(1:flen)  
...
c$$$         write(92,13) 'ENDDO'
c$$$      endif
c$$$      write(92,11) '    ANS(IPROC)=ANS(IPROC)+T*HWGT'
c$$$      if (dipol.le.nrc .and. offdiag)
c$$$     &     write(92,11) '    ANS(IPROC+1)=ANS(IPROC+1)+T1*HWGT'
c$$$      if (.not.standalone) then
c$$$         WRITE(92,11) 'ENDIF'
c$$$         WRITE(92,11) 'ENDDO'
c$$$      endif
c$$$      if (.not.standalone) then
c$$$         write(92,11) 'IF (ISUM_HEL .EQ. 1) THEN'
c$$$         write(92,11) '    WRITE(HEL_BUFF(1),''(16i5)'')' //
c$$$     &        '(NHEL(i,IHEL),i=1,nexternal-1)'
c$$$         if (offdiag)
c$$$     &        write(92,11) '    WRITE(HEL_BUFF(2),''(16i5)'')' //
c$$$     &        '(NHEL(i,IHEL+SKIP),i=1,nexternal-1)'
c$$$         write(92,11) 'ENDIF'
c$$$      endif
c$$$      WRITE(92,10) 'ENDIF'
c$$$
...
c      endif
      ...
      if (dipol.le.nrc .and. offdiag)
     &     write(92,10) 'ANS(IPROC+1)=ANS(IPROC+1)/DBLE(IDEN(IPROC))'
      write(92,10) 'ENDDO'
      write(92,10) 'calculatedBorn=.true.'
      write(92,10) 'END'
      write(92,10) ' '
      write(92,10) ' '
c
