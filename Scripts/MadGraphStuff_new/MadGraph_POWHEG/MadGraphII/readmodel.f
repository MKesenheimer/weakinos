!WTARO!: to read a spin-3/2 particle.
...
c
      integer i,j,n,jpart
      !WTARO integer info(0:4),ipart(0:5),itype(5),icoup(0:max_coup),icolor(8)
      integer info(0:4),ipart(0:6),itype(6),icoup(0:max_coup),icolor(8)
      integer ione,itwo,ithree
      integer pcolor(4,8)   !Puts particles numbers of singlet/triplet/oct
      character*20 ctemp
      integer itemp

ccccccccc WTARO cccccccccc flag for C-routine
	  integer flagC
...
c***********************CP3*******************************
c****************WTARO***********************************************
               elseif (j .eq. 2 .and. info_p(2,ipart(i)) .eq. 6) then
                   itype(2)=itype(2)-1 !VFermion not Fermion
                   itype(6)=itype(6)+1
c****************WTARO************************************************
...
c     FRS Vertex
c--------------------------------------------------------------------
         elseif(itype(2) .eq. 1 .and. itype(6) .eq. 1 
     &          .and. itype(3) .eq. 1) then !FRS
...
c            
            ...
            ipart(1) = inverse(ipart(1))
    		flagC = 1
            call addFRS1(ipart,words(1)(1:length(1)),
     &                   words(4)(1:length(4)),info,icoup,flagC)
           
c
c           Now add the reversed flow case (if its not already there)
c
          if (imaja.ne.2) then !WTARO: to remove unwanted diagrams.
            if (charge_c(ipart(2)) .ne. ipart(1)) then  
c               write(*,*) ipart(1),ipart(2),charge_c(ipart(2)) ,
c
c              Here we take CC of each component but also switch
c              order of two fermions since order gives fermion flow
c
...
c                       
            ipart(1) = inverse(ipart(1))
      		flagC = 1
            call addFRV1(ipart,words(1)(1:length(1)),
     &            words(4)(1:length(4)),info,icoup,flagC)
...
cend RF
c**************** WTARO start ****************************************
               else if (j .eq. 2 .and. info_p(2,ipart(i)) .eq. 6) then
                   itype(2)=itype(2)-1 !VFermion not Fermion
                   itype(6)=itype(6)+1
c**************** WTARO end ******************************************
...
c--------------------------------------------------------------------
         elseif(itype(2) .eq. 1 .and. itype(6) .eq. 1 
     &           .and. itype(1) .eq. 1 .and. itype(3) .eq. 1) then !FRVS
...
c     
            ...
            ipart(1) = inverse(ipart(1))
            flagC = 1
            call addFRVS1(ipart,words(1)(1:length(1)),
     &           words(5)(1:length(5)),info,icoup,flagC)
...
c     
            ...
            ipart(1) = inverse(ipart(1))
            flagC = 1
            call addFRVV1(ipart,words(1)(1:length(1)),
     &           words(5)(1:length(5)),info,icoup,flagC)
...
c     Set up the line type in info(4) Wavy,Solid,Dashed,Curly
            ...
            if (words(4)(1:1) .eq. 'R') info(4)=5 !WTARO: Wavy+Solid
...
c************CP3*************************
c***************************WTARO*******************************
            elseif(words(3)(1:1) .eq. 'R') then
               if(words(1)(1:3) .eq. 'gld') then
                  call addFermion(words(1)(1:length(1)),
     &              words(2)(1:length(2)),words(5)(1:length(5)),
     &              words(6)(1:length(6)),words(8)(1:length(8)),
     &              info(1),info(2),info(4),model)
               else                 
                  call addVFermion(words(1)(1:length(1)),
     &              words(2)(1:length(2)),words(5)(1:length(5)),
     &              words(6)(1:length(6)),words(8)(1:length(8)),
     &              info(1),info(2),info(4),model)
               endif
