!WTARO!: to add a vertex of a spin-3/2 particle.
...
c********************************* WTARO start *************************
      subroutine addFRVS1(ipart,prtcl1,coup,info,icoup,flagC)
c**************************************************************************
c     Sets up the FRVS vertex with all the permutations
c     info is the color info for the FRVS configuration
c     coup is coupling 
c     Order of particles should be FRVS
c     icoup is the coupling constant
c*************************************************************************
      implicit none
...
! Constants
      ...
      include 'params.inc'
      character*(*) IORVS_head                          
      parameter    (IORVS_head='IORVSX(W(1,1???),W(1,2???),'//
     &                               'W(1,3???),W(1,4???),')
      character*(*) IORVS_tail                          
      parameter    (IORVS_tail=',AMP(????))')
      character*(*) FVSOR_head 
      parameter    (FVSOR_head='FVSORX(W(1,2???),W(1,3???),W(1,4???),')
      character*(*) FVSOR_tail 
      parameter    (FVSOR_tail=',W(1,1???))')
      character*(*) JSIOR_head 
      parameter    (JSIOR_head='JSIORX(W(1,1???),W(1,2???),W(1,4???),')
      character*(*) JSIOR_tail 
      parameter    (JSIOR_tail=',W(1,3???))')
       character*(*) HVIOR_head 
      parameter    (HVIOR_head='HVIORX(W(1,1???),W(1,2???),W(1,3???),')
      character*(*) HVIOR_tail 
      parameter    (HVIOR_tail=',W(1,4???))')
      character*(*) IROVS_head                          
      parameter    (IROVS_head='IROVSX(W(1,1???),W(1,2???),'//
     &                               'W(1,3???),W(1,4???),')
      character*(*) IROVS_tail                          
      parameter    (IROVS_tail=',AMP(????))')
      character*(*) FVSIR_head 
      parameter    (FVSIR_head='FVSIRX(W(1,1???),W(1,3???),W(1,4???),')
      character*(*) FVSIR_tail 
      parameter    (FVSIR_tail=',W(1,2???))')
      character*(*) JSIRO_head 
      parameter    (JSIRO_head='JSIROX(W(1,1???),W(1,2???),W(1,4???),')
      character*(*) JSIRO_tail 
      parameter    (JSIRO_tail=',W(1,3???))')
       character*(*) HVIRO_head 
      parameter    (HVIRO_head='HVIROX(W(1,1???),W(1,2???),W(1,3???),')
      character*(*) HVIRO_tail 
      parameter    (HVIRO_tail=',W(1,4???))')
      
! Arguments

      character*(*) coup,prtcl1
      integer info(0:4),icoup(0:max_coup),ipart(0:5)

! Local
      
      integer ipart2(0:5),i
      character*(8) fmass,fwidth,vmass,vwidth,ymass,ywidth,smass,swidth
      character*(max_string) buff
cccccccc WTARO cccccccccc
	  integer flagC
	  integer porder
...
c     Calculate amplitude
c
      if ((prtcl1 .ne. 'gro').and.(prtcl1 .ne. 'gld')) then
      	porder = 1
      else
      	porder = -1
      endif
      ...
      do i=0,5
         ipart2(i)=ipart(i)
      enddo
      buff = IORVS_head//coup//IORVS_tail
      call add4vertex(ipart2,buff,info,icoup)  !Send the IORVS version
c
c
      do i=0,5
         ipart2(i)=ipart(i)
...
c
      ...
      vmass  = str(2,ipart(3))
      vwidth = str(3,ipart(3))
      buff = JSIOR_head//coup//','//vmass//','//
     $     vwidth//JSIOR_tail

      call add4vertex(ipart2,buff,info,icoup)

c
c     Now setup to calculate scalar particle 
c
     ...
     $     swidth//HVIOR_tail
      ...
      do i=0,5
         ipart2(i)=ipart(i)
      enddo
      buff = IROVS_head//coup//IROVS_tail
      call add4vertex(ipart2,buff,info,icoup)  !Send the IROVS version
c
c
      do i=0,5
         ipart2(i)=ipart(i)
...
c
     ...
     $     swidth//HVIRO_tail
      ...
      subroutine addFRVV1(ipart,prtcl1,coup,info,icoup,flagC)
c**************************************************************************
c     Sets up the FRVV vertex with all the permutations
c     info is the color info for the FRVV configuration
c     coup is coupling 
c     Order of particles should be FRVV
c     icoup is the coupling constant
c*************************************************************************
      implicit none
...
! Constants
      ...
      include 'params.inc'
      character*(*) IORVV_head                          
      parameter    (IORVV_head='IORVVX(W(1,1???),W(1,2???),'//
     &                               'W(1,3???),W(1,4???),')
      character*(*) IORVV_tail                          
      parameter    (IORVV_tail=',AMP(????))')
      character*(*) FVVOR_head 
      parameter    (FVVOR_head='FVVORX(W(1,2???),W(1,3???),W(1,4???),')
      character*(*) FVVOR_tail 
      parameter    (FVVOR_tail=',W(1,1???))')
      character*(*) JVIOR_head 
      parameter    (JVIOR_head='JVIORX(W(1,1???),W(1,2???),W(1,4???),')
      character*(*) JVIOR_tail 
      parameter    (JVIOR_tail=',W(1,3???))')
      character*(*) IROVV_head                          
      parameter    (IROVV_head='IROVVX(W(1,1???),W(1,2???),'//
     &                               'W(1,3???),W(1,4???),')
      character*(*) IROVV_tail                          
      parameter    (IROVV_tail=',AMP(????))')
      character*(*) FVVIR_head 
      parameter    (FVVIR_head='FVVIRX(W(1,1???),W(1,3???),W(1,4???),')
      character*(*) FVVIR_tail 
      parameter    (FVVIR_tail=',W(1,2???))')
      character*(*) JVIRO_head 
      parameter    (JVIRO_head='JVIROX(W(1,1???),W(1,2???),W(1,4???),')
      character*(*) JVIRO_tail 
      parameter    (JVIRO_tail=',W(1,3???))')
...
c
      ...
      do i=0,5
         ipart2(i)=ipart(i)
      enddo
      buff = IORVV_head//coup//IORVV_tail
      call add4vertex(ipart2,buff,info,icoup)  !Send the IORVV version
c
c
      do i=0,5
         ipart2(i)=ipart(i)
      ...
      buff = FVVOR_head//coup//','//fmass//','
     $     //fwidth//FVVOR_tail
      call add4vertex(ipart2,buff,info,icoup)  
c
c     Now setup to calculate vector field particle 1
c
      do i=0,5
         ipart2(i)=ipart(i)
      enddo
     ...
     $     vwidth//JVIOR_tail
      ...
      do i=0,5
         ipart2(i)=ipart(i)
      enddo
      buff = IROVV_head//coup//IROVV_tail
      call add4vertex(ipart2,buff,info,icoup)  !Send the IROVV version
c
c
      do i=0,5
         ipart2(i)=ipart(i)
