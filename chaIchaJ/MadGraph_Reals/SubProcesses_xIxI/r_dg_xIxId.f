      SUBROUTINE SMATRIX_DG_XIXID(P1,ANS)
C  
...
C  
C FOR PROCESS : d g -> xI+ xI- d  
C  
C Crossing   1 is d g -> xI+ xI- d  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C  
      ...
      common/to_amps_dg_xixi/  amp2,       jamp2
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF(GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATRIX_DG_XIXID(P,NHEL(1,IHEL),JC(1))            
      ...
      REAL*8 FUNCTION MATRIX_DG_XIXID(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
c delete resonant diagrams (diagram removal type I)
     ...
     &             +AMP(   6)+AMP(   7)
      MATRIX_DG_XIXID = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP =(0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATRIX_DG_XIXID =MATRIX_DG_XIXID
...
c (diagram removal type II)
     ...
     &                      +ZTEMP*DCONJG(JAMPR(I))/DENOM(I)   
      ...
      MATRIX_DG_XIXID = MATRIX_DG_XIXID - MATRIX_RESONANT
#endif
...
c back in subroutine setosresreal
      ...
      MATRIX_DG_XIXID = MATRIX_DG_XIXID
     &                - MATRIX_DG_XIXID_RES(p,NHEL,IC,"ul35")
#endif
...
c the interference term and the off-shell contributions
      ...
      if( (S.ge.(MUL+MXI)**2) .and. (MUL.ge.MXI)) then
        ...
        COUNTER35L = RATIO35L*MATRIX_DG_XIXID_RES(p_OS,NHEL,IC,"ul35") ! generate the counter term
      endif
      
      MATRIX_DG_XIXID = MATRIX_DG_XIXID - COUNTER35L
#endif
      
      ! amp2 and jamp2 aren't used
      Do I = 1, NGRAPHS
