      SUBROUTINE SMATRIX_GD_XIXID(P1,ANS)
C  
...
C  
      ...
      common/to_amps_gd_xixi/  amp2,       jamp2
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF(GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATRIX_GD_XIXID(P,NHEL(1,IHEL),JC(1))            
      ...
      REAL*8 FUNCTION MATRIX_GD_XIXID(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
#ifdef DR_I
      AMP(2)  = 0 ! diagram 2,  s35, L, XI+
      AMP(3)  = 0 ! diagram 3,  s35, L, XI+
#endif
      
      JAMP(   1) = -AMP(   1)-AMP(   2)-AMP(   3)+AMP(   4)+AMP(   5)
     &             +AMP(   6)+AMP(   7)
      MATRIX_GD_XIXID = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP =(0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATRIX_GD_XIXID =MATRIX_GD_XIXID
...
c (diagram removal type II)
     ...
     &                      +ZTEMP*DCONJG(JAMPR(I))/DENOM(I)   
      ...
      MATRIX_GD_XIXID = MATRIX_GD_XIXID - MATRIX_RESONANT
#endif
...
c back in subroutine setosresreal
      ...
      MATRIX_GD_XIXID = MATRIX_GD_XIXID
     &                  - MATRIX_GD_XIXID_RES(p,NHEL,IC,"ul35")
#endif
...
c the interference term and the off-shell contributions
      ...
      if( (S.ge.(MUL+MXI)**2) .and. (MUL.ge.MXI)) then
        ...
        COUNTER35L = RATIO35L*MATRIX_GD_XIXID_RES(p_OS,NHEL,IC,"ul35") ! generate the counter term
      endif

      MATRIX_GD_XIXID = MATRIX_GD_XIXID - COUNTER35L
#endif
