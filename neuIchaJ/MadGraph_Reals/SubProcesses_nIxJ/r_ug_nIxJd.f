      SUBROUTINE SMATRIX_UG_NIXJD(P1,ANS)
C  
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF(GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATRIX_UG_NIXJD(P,NHEL(1,IHEL),JC(1))            
      ...
      REAL*8 FUNCTION MATRIX_UG_NIXJD(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      ! diagram 2 <- resonant
      CALL HVSXXX(W(1,2),W(1,6),-GC,MUL,WULR,W(1,8))    
      ...
      CALL HIOXXX(W(1,1),W(1,13),GDLXJM,MDL,WDL,W(1,15))                                                          
      CALL FVICXX(W(1,14),W(1,2),GG,ZERO,ZERO,W(1,16))    
      CALL IOSCXX(W(1,16),W(1,3),W(1,15),GDLNIP,AMP(6))        
      ! diagram 7 <- resonant
      CALL HIOCXX(W(1,14),W(1,3),GDLNIP,MDL,WDLR,W(1,17))                                                          
      CALL VSSXXX(W(1,2),W(1,17),W(1,15),GC,AMP(7))             
      ! diagram 8 <- resonant
...
c delete resonant diagrams (diagram removal type I)
     ...
     &             +AMP(   6)+AMP(   7)+AMP(   8)
      MATRIX_UG_NIXJD = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP =(0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATRIX_UG_NIXJD =MATRIX_UG_NIXJD
...
c (diagram removal type II)
     ...
     &                      +ZTEMP*DCONJG(JAMPR(I))/DENOM(I)   
      ...
      MATRIX_UG_NIXJD = MATRIX_UG_NIXJD - MATRIX_RESONANT
#endif
...
c back in subroutine setosresreal
      ...
      MATRIX_UG_NIXJD = MATRIX_UG_NIXJD
     &                - MATRIX_UG_NIXJD_RES(p,NHEL,IC,"dl35")
     &                - MATRIX_UG_NIXJD_RES(p,NHEL,IC,"ul45")
!     &                - MATRIX_UG_NIXJD_RES(p,NHEL,IC,"allr")
#endif
...
c the interference term and the off-shell contributions
      ...
      MATRIX_UG_NIXJD = MATRIX_UG_NIXJD - COUNTER35L - COUNTER45L
#endif
