      SUBROUTINE SMATRIX_GD_NIXJU(P1,ANS)
C  
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF(GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATRIX_GD_NIXJU(P,NHEL(1,IHEL),JC(1))            
      ...
      REAL*8 FUNCTION MATRIX_GD_NIXJU(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL FSOCXX(W(1,4),W(1,6),GDLXJP,ZERO,ZERO,W(1,7))                                                          
      CALL IOVCXX(W(1,5),W(1,7),W(1,1),GG,AMP(1))            
      ! diagram 2 <- resonant
      CALL HVSXXX(W(1,1),W(1,6),-GC,MDL,WDLR,W(1,8))    
      CALL IOSCXX(W(1,5),W(1,4),W(1,8),GDLXJP,AMP(2))        
      ! diagram 3 <- resonant
      CALL FVIXXX(W(1,2),W(1,1),GG,ZERO,ZERO,W(1,9))     
      CALL HIOXXX(W(1,9),W(1,3),GDLNIM,MDL,WDLR,W(1, 10))                                                          
      CALL IOSCXX(W(1,5),W(1,4),W(1,10),GDLXJP,AMP(3))        
      ! diagram 4
      CALL HIOXXX(W(1,2),W(1,4),GULXJM,MUL,WUL,W(1, 11))                                                          
      CALL FVICXX(W(1,5),W(1,1),GG,ZERO,ZERO,W(1,12))    
      CALL IOSCXX(W(1,12),W(1,3),W(1,11),GULNIP,AMP(4))        
      ! diagram 5 <- resonant
      CALL HIOCXX(W(1,5),W(1,3),GULNIP,MUL,WULR,W(1,13))                                                          
      CALL VSSXXX(W(1,1),W(1,13),W(1,11),GC,AMP(5))             
      ! diagram 6 <- resonant
      CALL HIOXXX(W(1,9),W(1,4),GULXJM,MUL,WULR,W(1, 14))                                                          
      CALL IOSCXX(W(1,5),W(1,3),W(1,14),GULNIP,AMP(6))        
      ! diagram 7
      CALL IXXXXX(P(0,4),MXJ,NHEL(4),-1*IC(4),W(1,15))         
      CALL OXXXXX(P(0,5),ZERO,NHEL(5),+1*IC(5),W(1,16))        
      CALL JIOCXX(W(1,15),W(1,3),GWNIXJ,WMASS,WWIDTH,W(1,17))                                                          
      CALL FVOXXX(W(1,16),W(1,1),GG,ZERO,ZERO,W(1,18))     
      CALL IOVXXX(W(1,2),W(1,18),W(1,17),GWF,AMP(7))            
      ! diagram 8
      CALL IOVXXX(W(1,9),W(1,16),W(1,17),GWF,AMP(8))            
...
c delete resonant diagrams (diagram removal type I)
     ...
     &             +AMP(   6)-AMP(   7)-AMP(   8)
      MATRIX_GD_NIXJU = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP =(0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATRIX_GD_NIXJU =MATRIX_GD_NIXJU
...
c (diagram removal type II)
     ...
     &                      +ZTEMP*DCONJG(JAMPR(I))/DENOM(I)   
      ...
      MATRIX_GD_NIXJU = MATRIX_GD_NIXJU - MATRIX_RESONANT
#endif
...
c back in subroutine setosresreal
      ...
      MATRIX_GD_NIXJU = MATRIX_GD_NIXJU
     &                - MATRIX_GD_NIXJU_RES(p,NHEL,IC,"ul35")
     &                - MATRIX_GD_NIXJU_RES(p,NHEL,IC,"dl45")
!     &                - MATRIX_GD_NIXJU_RES(p,NHEL,IC,"allr")
#endif
...
c the interference term and the off-shell contributions
      ...
      if( (S.ge.(MUL+MXJ)**2) .and. (MUL.ge.dabs(MNI))) then
        ...
        COUNTER35L = RATIO35L*MATRIX_GD_NIXJU_RES(p_OS,NHEL,IC,"ul35") ! generate the counter term
      endif
      ! S45 = MDL^2, m4 = MXJ, m5 = 0D0
      if( (S.ge.(MDL+dabs(MNI))**2) .and. (MDL.ge.MXJ)) then
        call off_to_on(p,"dl45",p_OS)
        RATIO45L = (MDL*WREG)**2/((S45-MDL**2)**2+(MDL*WREG)**2)
        COUNTER45L = RATIO45L*MATRIX_GD_NIXJU_RES(p_OS,NHEL,IC,"dl45")
      endif
      
      MATRIX_GD_NIXJU = MATRIX_GD_NIXJU - COUNTER35L - COUNTER45L
#endif
