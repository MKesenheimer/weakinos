      SUBROUTINE SMATRIX_UXG_NIXJDX_RES(P1,CHAN,ANS)
C  
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF(GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATRIX_UXG_NIXJDX_RES(P,NHEL(1,IHEL),JC(1),CHAN)            
      ...
      REAL*8 FUNCTION MATRIX_UXG_NIXJDX_RES(P,NHEL,IC,CHAN)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      if(CHAN.eq."dl35") then
        JAMP(1) = -AMP(2)-AMP(3)
      ! S45 = MUL^2
      elseif (CHAN.eq."ul45") then
        JAMP(1) = +AMP(7)+AMP(8)
      ! both resonant channels
      elseif (CHAN.eq."allr") then
        JAMP(1) = -AMP(2)-AMP(3)+AMP(7)+AMP(8)
      else
        !print*,"error in r_uxg_nixjdx_res.f"
        !stop
        JAMP(1) = 1D-99
      endif
      
      MATRIX_UXG_NIXJDX_RES = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP =(0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATRIX_UXG_NIXJDX_RES =MATRIX_UXG_NIXJDX_RES
