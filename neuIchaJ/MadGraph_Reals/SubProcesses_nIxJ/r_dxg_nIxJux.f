      SUBROUTINE SMATRIX_DXG_NIXJUX(P1,ANS)
C  
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF(GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATRIX_DXG_NIXJUX(P,NHEL(1,IHEL),JC(1))            
      ...
      REAL*8 FUNCTION MATRIX_DXG_NIXJUX(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL IOVCXX(W(1,15),W(1,13),W(1,2),GG,AMP(6))            
      ! diagram 7 <- resonant
      CALL HVSXXX(W(1,2),W(1,14),GC,MDL,WDLR,W(1,16))     
...
c delete resonant diagrams (diagram removal type I)
     ...
     &             -AMP(   6)-AMP(   7)-AMP(   8)
      MATRIX_DXG_NIXJUX = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP =(0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATRIX_DXG_NIXJUX =MATRIX_DXG_NIXJUX
...
c (diagram removal type II)
     ...
     &                      +ZTEMP*DCONJG(JAMPR(I))/DENOM(I)   
      ...
      MATRIX_DXG_NIXJUX = MATRIX_DXG_NIXJUX - MATRIX_RESONANT
#endif
...
c back in subroutine setosresreal
      ...
      MATRIX_DXG_NIXJUX = MATRIX_DXG_NIXJUX
     &                  - MATRIX_DXG_NIXJUX_RES(p,NHEL,IC,"ul35")
     &                  - MATRIX_DXG_NIXJUX_RES(p,NHEL,IC,"dl45")
!     &                  - MATRIX_DXG_NIXJUX_RES(p,NHEL,IC,"allr")
#endif
...
c the interference term and the off-shell contributions
      ...
      if( (S.ge.(MUL+MXJ)**2) .and. (MUL.ge.dabs(MNI))) then
        ...
        COUNTER35L = RATIO35L*MATRIX_DXG_NIXJUX_RES(p_OS,NHEL,IC,"ul35") ! generate the counter term
      endif
      ...
      if( (S.ge.(MDL+dabs(MNI))**2) .and. (MDL.ge.MXJ)) then
        ...
        COUNTER45L = RATIO45L*MATRIX_DXG_NIXJUX_RES(p_OS,NHEL,IC,"dl45")
      endif
      
      MATRIX_DXG_NIXJUX = MATRIX_DXG_NIXJUX - COUNTER35L - COUNTER45L
#endif
