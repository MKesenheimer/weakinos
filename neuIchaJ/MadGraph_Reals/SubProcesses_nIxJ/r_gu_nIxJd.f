      SUBROUTINE SMATRIX_GU_NIXJD(P1,ANS)
C  
...
C  
C FOR PROCESS : g u -> ni xj+ d  
C  
C Crossing   1 is g u -> ni xj+ d  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF(GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATRIX_GU_NIXJD(P,NHEL(1,IHEL),JC(1))            
      ...
      REAL*8 FUNCTION MATRIX_GU_NIXJD(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL HIOXXX(W(1,9),W(1,3),GULNIM,MUL,WULR,W(1,10))                                                          
      ...
      CALL HIOXXX(W(1,2),W(1,13),GDLXJM,MDL,WDL,W(1,15))                                                          
      CALL FVICXX(W(1,14),W(1,1),GG,ZERO,ZERO,W(1,16))    
      CALL IOSCXX(W(1,16),W(1,3),W(1,15),GDLNIP,AMP(6))        
      ! diagram 7 <- resonant
      CALL HIOCXX(W(1,14),W(1,3),GDLNIP,MDL,WDLR,W(1,17))                                                          
      CALL VSSXXX(W(1,1),W(1,17),W(1,15),GC,AMP(7))             
      ! diagram 8 <- resonant
      CALL HIOXXX(W(1,9),W(1,13),GDLXJM,MDL,WDLR,W(1,18))                                                          
      CALL IOSCXX(W(1,14),W(1,3),W(1,18),GDLNIP,AMP(8))       
...
c delete resonant diagrams (diagram removal type I)
     ...
     &             +AMP(   6)+AMP(   7)+AMP(   8)
      MATRIX_GU_NIXJD = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP =(0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATRIX_GU_NIXJD =MATRIX_GU_NIXJD
...
c (diagram removal type II)
#if defined(DR_II)
      JAMPR(1)  = +AMP(   2)+AMP(   3)+AMP(   7)+AMP(   8)
     ...
     &                      +ZTEMP*DCONJG(JAMPR(I))/DENOM(I)   
      ...
      MATRIX_GU_NIXJD = MATRIX_GU_NIXJD - MATRIX_RESONANT
#endif
...
c back in subroutine setosresreal
      ...
      MATRIX_GU_NIXJD = MATRIX_GU_NIXJD
     &                - MATRIX_GU_NIXJD_RES(p,NHEL,IC,"dl35")
     &                - MATRIX_GU_NIXJD_RES(p,NHEL,IC,"ul45")
!     &                - MATRIX_GU_NIXJD_RES(p,NHEL,IC,"allr")
#endif
...
c the interference term and the off-shell contributions
      ...
      if( (S.ge.(MDL+MXJ)**2) .and. (MDL.ge.dabs(MNI))) then
        ...
        COUNTER35L = RATIO35L*MATRIX_GU_NIXJD_RES(p_OS,NHEL,IC,"dl35") ! generate the counter term
      endif
      ...
      if( (S.ge.(MUL+dabs(MNI))**2) .and. (MUL.ge.MXJ)) then
        ...
        COUNTER45L = RATIO45L*MATRIX_GU_NIXJD_RES(p_OS,NHEL,IC,"ul45")
      endif
      
      MATRIX_GU_NIXJD = MATRIX_GU_NIXJD - COUNTER35L - COUNTER45L
#endif
