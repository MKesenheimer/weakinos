      SUBROUTINE SMATRIX_GUX_NIXJDX(P1,ANS)
C  
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF(GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATRIX_GUX_NIXJDX(P,NHEL(1,IHEL),JC(1))            
      ...
      REAL*8 FUNCTION MATRIX_GUX_NIXJDX(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      ! diagram 2 <- resonant
      CALL HIOXXX(W(1,5),W(1,3),GDLNIM,MDL,WDLR,W(1, 8))                                                          
      CALL VSSXXX(W(1,1),W(1,6),W(1,8),GC,AMP(2))           
      ! diagram 3 <- resonant
      CALL FVOXXX(W(1,2),W(1,1),GG,ZERO,ZERO,W(1,9))     
      CALL HIOXXX(W(1,4),W(1,9),GDLXJP,MDL,WDLR,W(1, 10))                                                          
      CALL IOSXXX(W(1,5),W(1,3),W(1,10),GDLNIM,AMP(3))         
      ! diagram 4
      CALL JIOCXX(W(1,4),W(1,3),GWNIXJ,WMASS,WWIDTH,W(1,11))                                                          
      ...
      ! diagram 7 <- resonant
      CALL HVSXXX(W(1,1),W(1,14),GC,MUL,WULR,W(1,16))     
      CALL IOSXXX(W(1,5),W(1,13),W(1,16),GULXJM,AMP(7))         
      ! diagram 8 <- resonant
      CALL HIOXXX(W(1,12),W(1,9),GULNIP,MUL,WULR,W(1, 17))                                                          
...
c delete resonant diagrams (diagram removal type I)
     ...
     &             +AMP(   6)+AMP(   7)+AMP(   8)
      MATRIX_GUX_NIXJDX = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP =(0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATRIX_GUX_NIXJDX =MATRIX_GUX_NIXJDX
...
c (diagram removal type II)
     ...
     &                      +ZTEMP*DCONJG(JAMPR(I))/DENOM(I)   
      ...
      MATRIX_GUX_NIXJDX = MATRIX_GUX_NIXJDX - MATRIX_RESONANT
#endif
...
c back in subroutine setosresreal
      ...
      MATRIX_GUX_NIXJDX = MATRIX_GUX_NIXJDX
     &                  - MATRIX_GUX_NIXJDX_RES(p,NHEL,IC,"dl35")
     &                  - MATRIX_GUX_NIXJDX_RES(p,NHEL,IC,"ul45")
!     &                  - MATRIX_GUX_NIXJDX_RES(p,NHEL,IC,"allr")
#endif
...
c the interference term and the off-shell contributions
      ...
      if( (S.ge.(MDL+MXJ)**2) .and. (MDL.ge.dabs(MNI))) then
        ...
        COUNTER35L = RATIO35L*MATRIX_GUX_NIXJDX_RES(p_OS,NHEL,IC,"dl35") ! generate the counter term
      endif
      ...
      if( (S.ge.(MUL+dabs(MNI))**2) .and. (MUL.ge.MXJ)) then
        ...
        COUNTER45L = RATIO45L*MATRIX_GUX_NIXJDX_RES(p_OS,NHEL,IC,"ul45")
      endif
      
      MATRIX_GUX_NIXJDX = MATRIX_GUX_NIXJDX - COUNTER35L - COUNTER45L
#endif
