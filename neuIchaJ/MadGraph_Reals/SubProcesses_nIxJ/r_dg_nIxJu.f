      SUBROUTINE SMATRIX_DG_NIXJU(P1,ANS)
C  
...
C  
C FOR PROCESS : d g -> ni xj- u  
C  
C Crossing   1 is d g -> ni xj- u  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF(GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATRIX_DG_NIXJU(P,NHEL(1,IHEL),JC(1))            
      ...
      REAL*8 FUNCTION MATRIX_DG_NIXJU(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      ! diagram 2 <- resonant
      CALL HIOCXX(W(1,5),W(1,3),GULNIP,MUL,WULR,W(1,8))                                                          
      ...
      CALL IOSCXX(W(1,5),W(1,3),W(1,10),GULNIP,AMP(3))        
      ! diagram 4
      CALL HIOXXX(W(1,1),W(1,3),GDLNIM,MDL,WDL,W(1, 11))                                                          
      CALL FSOCXX(W(1,4),W(1,11),GDLXJP,ZERO,ZERO,W(1,12))                                                          
      CALL IOVCXX(W(1,5),W(1,12),W(1,2),GG,AMP(4))            
      ! diagram 5 <- resonant
      CALL HVSXXX(W(1,2),W(1,11),-GC,MDL,WDLR,W(1,13))    
      CALL IOSCXX(W(1,5),W(1,4),W(1,13),GDLXJP,AMP(5))        
      ! diagram 6 <- resonant
      CALL HIOXXX(W(1,9),W(1,3),GDLNIM,MDL,WDLR,W(1, 14))                                                          
      CALL IOSCXX(W(1,5),W(1,4),W(1,14),GDLXJP,AMP(6))      
      ! diagram 7
      ...
      CALL FVOXXX(W(1,16),W(1,2),GG,ZERO,ZERO,W(1,18))     
      CALL IOVXXX(W(1,1),W(1,18),W(1,17),GWF,AMP(7))        
      ! diagram 8
...
c delete resonant diagrams (diagram removal type I)
     ...
     &             -AMP(   6)-AMP(   7)-AMP(   8)
      MATRIX_DG_NIXJU = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP =(0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATRIX_DG_NIXJU =MATRIX_DG_NIXJU
...
c (diagram removal type II)
     ...
     &                      +ZTEMP*DCONJG(JAMPR(I))/DENOM(I)   
      ...
      MATRIX_DG_NIXJU = MATRIX_DG_NIXJU - MATRIX_RESONANT
#endif
...
c back in subroutine setosresreal
      ...
      MATRIX_DG_NIXJU = MATRIX_DG_NIXJU
     &                - MATRIX_DG_NIXJU_RES(p,NHEL,IC,"ul35")
     &                - MATRIX_DG_NIXJU_RES(p,NHEL,IC,"dl45")
!     &                - MATRIX_DG_NIXJU_RES(p,NHEL,IC,"allr")
#endif
...
c the interference term and the off-shell contributions
      ...
      ! on_shell_jac(shat,mi,mj,mk,sij,mij)
      if( (S.ge.(MUL+MXJ)**2) .and. (MUL.ge.dabs(MNI))) then
        ...
        COUNTER35L = RATIO35L*MATRIX_DG_NIXJU_RES(p_OS,NHEL,IC,"ul35") ! generate the counter term
      endif
      ...
      if( (S.ge.(MDL+dabs(MNI))**2) .and. (MDL.ge.MXJ)) then
        ...
        COUNTER45L = RATIO45L*MATRIX_DG_NIXJU_RES(p_OS,NHEL,IC,"dl45")
      endif
      
      MATRIX_DG_NIXJU = MATRIX_DG_NIXJU - COUNTER35L - COUNTER45L
#endif
