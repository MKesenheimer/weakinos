      SUBROUTINE SMATRIX_UXG_NIXJDX(P1,ANS)
C  
...
C  
C FOR PROCESS : u~ g -> ni xj- d~  
C  
C Crossing   1 is u~ g -> ni xj- d~  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF(GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATRIX_UXG_NIXJDX(P,NHEL(1,IHEL),JC(1))            
      ...
      REAL*8 FUNCTION MATRIX_UXG_NIXJDX(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      ! diagram 7 <- resonant
      CALL HVSXXX(W(1,2),W(1,14),GC,MUL,WULR,W(1,16))     
...
c delete resonant diagrams (diagram removal type I)
     ...
     &             +AMP(   6)+AMP(   7)+AMP(   8)
      MATRIX_UXG_NIXJDX = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP =(0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATRIX_UXG_NIXJDX =MATRIX_UXG_NIXJDX
...
c (diagram removal type II)
     ...
     &                      +ZTEMP*DCONJG(JAMPR(I))/DENOM(I)   
      ...
      MATRIX_UXG_NIXJDX = MATRIX_UXG_NIXJDX - MATRIX_RESONANT
#endif
...
c back in subroutine setosresreal
      ...
      MATRIX_UXG_NIXJDX = MATRIX_UXG_NIXJDX
     &                  - MATRIX_UXG_NIXJDX_RES(p,NHEL,IC,"dl35")
     &                  - MATRIX_UXG_NIXJDX_RES(p,NHEL,IC,"ul45")
!     &                  - MATRIX_UXG_NIXJDX_RES(p,NHEL,IC,"allr")
#endif
...
c the interference term and the off-shell contributions
      ...
      if( (S.ge.(MDL+MXJ)**2) .and. (MDL.ge.dabs(MNI))) then
        ...
        COUNTER35L = RATIO35L*MATRIX_UXG_NIXJDX_RES(p_OS,NHEL,IC,"dl35") ! generate the counter term
      endif
      ...
      if( (S.ge.(MUL+dabs(MNI))**2) .and. (MUL.ge.MXJ)) then
        ...
        COUNTER45L = RATIO45L*MATRIX_UXG_NIXJDX_RES(p_OS,NHEL,IC,"ul45")
      endif
      
      MATRIX_UXG_NIXJDX = MATRIX_UXG_NIXJDX - COUNTER35L - COUNTER45L
#endif
